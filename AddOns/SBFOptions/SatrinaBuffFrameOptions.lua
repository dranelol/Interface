-- $Date: 2012-09-12 04:50:09 +0000 (Wed, 12 Sep 2012) $
...
SBFOptions.EnableTab = function(self, tab, enabled)
  if enabled then
    tab.text:SetTextColor(1,1,1)
    tab.disabled = false
    tab:SetHighlightTexture("Interface\FriendsFrame\UI-FriendsFrame-HighlightBar", "ADD")
  else
    tab.text:SetTextColor(0.5,0.5,0.5)
    tab.disabled = true
    tab:SetHighlightTexture(nil, nil)
  end
end
...
SBFOptions.SelectFrame = function(self, f)
  ...
  if var.general.interactiveFrame then
    self:EnableTab(SBFOFilterConfigButton, false)
    self:EnableTab(SBFOSpellsConfigButton, false)
  else
