...
function ArkInventory.VaultInfoSet( )
	...
	if n then
		...
		local id = string.format( "%s%s%s%s", ArkInventory.Const.GuildTag, n, ArkInventory.Const.PlayerIDTag, cp.realm )
...
function ArkInventory:LISTEN_BANK_SLOT( )
	...
	ArkInventory.Frame_Main_Generate( ArkInventory.Const.Location.Bank, ArkInventory.Const.Window.Draw.Init )
...
function ArkInventory.ScanBattlePet( )
	...
	if ( not ArkInventory.PetJournal.IsReady( ) ) then
		--ArkInventory.Output( "journal not ready" )
		ArkInventory:SendMessage( "LISTEN_PETJOURNAL_RELOAD_BUCKET", "RESCAN" )
		return
	end
	
	if ( ArkInventory.PetJournal.NumPets( ) == 0 ) then
