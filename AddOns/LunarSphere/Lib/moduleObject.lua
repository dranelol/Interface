...
function Lunar.Object:Create(objectType, objectName, objectParent, objectTitle, objectWidth, objectHeight, foregroundR, foregroundG, foregroundB, backgroundR, backgroundG, backgroundB, noAutoClose)
	...
	if (objectType == "window") then
		...
		if (objectHeight) then
			getglobal(objectName .. "WindowContainer"):SetHeight(objectHeight - 20);
		end

		-- Set the color of the window's background, if specified
		if ((backgroundR) and (backgroundG) and (backgroundB)) then
			getglobal(objectName .. "WindowContainer"):SetBackdropColor(backgroundR, backgroundG, backgroundB, 1.0);
		end

		-- Set the window's title text
		if (objectTitle) then
			getglobal(objectName .. "Title"):SetText(objectTitle);
		end
...
--[[	-- Object type: Check box
	if (objectType == "checkbox") then
		...
		getglobal(tempObject:GetName() .. "Text"):SetText(objectTitle);
...
function Lunar.Object:CreateCheckbox(xLoc, yLoc, objectText, objectSetting, enabledValue, objectParent, settingUpdateFunction)
	...
	getglobal(tempObject:GetName() .. "Text"):SetFont((select(1, GameFontNormal:GetFont())), 10); --Fonts\\FRIZQT__.TTF", 10);
	...
	getglobal(tempObject:GetName() .. "Text"):SetPoint("Topleft", tempObject, "TopRight", 0, -5);
	...
	if (LunarSphereSettings[objectSetting] == enabledValue) then
		tempObject:SetChecked(1);
	else
		tempObject:SetChecked(0);
	end
...
--[[		tempObject:SetScript("OnClick",
		function ()
			LunarSphereSettings[objectSetting] = (this:GetChecked() == 1)
...
function Lunar.Object:CreateColorSelector(xLoc, yLoc, objectText, objectSetting, objectParent, colorChangeFunction, hasAlpha)
	...
	getglobal(tempObject:GetName() .. "Color"):SetTexture(
...
--			colorB = LunarSphereSettings[objectSetting][3];
		...
		function ()
			...
			colorChangeFunction();
			getglobal(tempObject:GetName() .. "Color"):SetTexture(r, g, b);
		end
...
function Lunar.Object:CreateSlider(xLoc, yLoc, objectText, objectSetting, objectParent, minValue, maxValue, valueStep, updateFunction, useTextBox)
	...
	if (useTextBox == true) then
		...
		getglobal(tempObject:GetName() .. "Value"):SetScript("OnEnterPressed", Lunar.Object.EditBox_OnEnterPressed);
--		getglobal(tempObject:GetName() .. "Value"):SetNumeric(true);
		getglobal(tempObject:GetName() .. "Value"):SetScript("OnEditFocusLost", Lunar.Object.EditBox_OnEnterPressed);
	else
...
--	tempObject:SetHeight(21);
	...
	getglobal(tempObject:GetName() .. "Text"):SetFont((select(1, GameFontNormal:GetFont())), 10); --Fonts\\FRIZQT__.TTF", 10);
	getglobal(tempObject:GetName() .. "Text"):SetText(objectText);
	getglobal(tempObject:GetName() .. "Text"):SetVertexColor(NORMAL_FONT_COLOR.r , NORMAL_FONT_COLOR.g , NORMAL_FONT_COLOR.b);
	...
	if (useTextBox == true) then
		getglobal(tempObject:GetName() .. "Value"):SetTextColor(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
	else
		getglobal(tempObject:GetName() .. "Value"):SetVertexColor(HIGHLIGHT_FONT_COLOR.r, HIGHLIGHT_FONT_COLOR.g, HIGHLIGHT_FONT_COLOR.b);
	end
	...
	if (LunarSphereSettings[objectSetting]) then
		if (useTextBox == true) then
			getglobal(tempObject:GetName() .. "Value"):SetText(tostring(floor((LunarSphereSettings[objectSetting] * 100) + 0.5) / 100));
		else
			getglobal(tempObject:GetName() .. "Value"):SetText(": " .. tostring(floor((LunarSphereSettings[objectSetting] * 100) + 0.5) / 100));
		end
		...
		if (useTextBox == true) then
			getglobal(tempObject:GetName() .. "Value"):SetText(tostring(floor((tempObject:GetValue() * 100) + 0.5) / 100));
		else
			getglobal(tempObject:GetName() .. "Value"):SetText(": " .. tostring(floor((tempObject:GetValue() * 100) + 0.5) / 100));
		end
	end
...
--[[	tempObject:SetScript("OnValueChanged", 
	function (self)
		if (LunarSphereSettings[self.objectSetting]) then
			...
			getglobal(self:GetName() .. "Value"):SetText(": " .. tostring(floor((LunarSphereSettings[self.objectSetting] * 100) + 0.5) / 100));
		else
			getglobal(self:GetName() .. "Value"):SetText(": " .. tostring(floor((self:GetValue() * 100) + 0.5) / 100));
		end
...
--	tempObject.displayMode = "MENU"
	...
	getglobal(tempObject:GetName() .. "Label"):SetText(objectText);
	tempObject:SetPoint("Topleft", xLoc, yLoc);
					...
					if (listName == "Button_Type") then
						...
						if (UIDropDownMenu_GetSelectedName(dropdownObject) == Lunar.Locale["BUTTON_MENU"]) then
							if (self:GetText() ~= Lunar.Locale["BUTTON_MENU"]) then
								getglobal("LSSettingsbuttonAction" .. clickType .. "Icon"):SetTexture("");
...
--							UIDropDownMenu_SetSelectedName(dropdownObject, self:GetText());
	...
	--							end
								...
								getglobal("LSSettingsbuttonAction" .. clickType .. "Icon"):SetTexture(objectTexture);
							end
							...
							if (buttonType >= 100) and (buttonType < 110)  then
								if (buttonType == 100) then
									...
									SetPortraitTexture(getglobal("LSSettingsbuttonAction" .. clickType .. "Icon"), "player");
								else
...
--							end
					...
					elseif (listName == "Gauge_Events") then 
						...
						getglobal(dropdownObject:GetName() .. "Text"):SetText(text);
						Lunar.Sphere:SetGaugeType(getglobal("LSSettingsgaugeName").currentGauge, eventID);
					else
--						UIDropDownMenu_SetSelectedName(dropdownObject, this:GetText());
						UIDropDownMenu_SetText(dropdownObject, self:GetText(), dropdownObject);
						UIDropDownMenu_SetSelectedName(dropdownObject, self:GetText());
						dropdownObject.selectedValue = self.value;
					end
...
function Lunar.Object:SubmenuFunction(listName, updateFunction)
	...
	if (type(UIDROPDOWNMENU_OPEN_MENU) == "table") then
		...
		UIDropDownMenu_SetSelectedName(getglobal(UIDROPDOWNMENU_OPEN_MENU), self:GetText());
		getglobal(UIDROPDOWNMENU_OPEN_MENU).selectedValue = self.value;
	end
...
--				end
	...
	elseif (listName == "Gauge_Events") then
		Lunar.Sphere:SetGaugeType(getglobal("LSSettingsgaugeName").currentGauge, self.value)
	end
...
function Lunar.Object:SkinDropDown(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList)
	...
	local backdrop = getglobal("DropDownList" .. UIDROPDOWNMENU_MENU_LEVEL .. "Backdrop");
	if not Lunar.Object.dropdownSkin then
		Lunar.Object.dropdownSkin = backdrop:GetBackdrop()
		Lunar.Object.dropdownSkinLS = getglobal("LSSetttingsGaugeOptionsContainer"):GetBackdrop()
...
function Lunar.Object.SetSetting(self)
	LunarSphereSettings[string.match(self:GetName(), "LSSettings(.*)")] = (self:GetChecked() == 1)
end

function Lunar.Object.SetRadioSetting(self)
	local i = 0;
	local settingName = string.match(self:GetName(), "LSSettings(.*)(%d)");
	object = getglobal("LSSettings" .. settingName .. "0");
	while (object) do 
		i = i + 1;
		object:SetChecked(nil);
		object = getglobal("LSSettings" .. settingName .. i);
	end
	self:SetChecked(1);
...
function Lunar.Object.Slider_OnValueChanged(self)
	...
	if (LunarSphereSettings[objectSetting]) then
		...
		if (self.hasTextBox == true) then
			getglobal(self:GetName() .. "Value"):SetText(tostring(floor((LunarSphereSettings[objectSetting] * 100) + 0.5) / 100));
			if (getglobal(self:GetName() .. "Value"):HasFocus()) then
				getglobal(self:GetName() .. "Value"):ClearFocus();
			end
		else
			getglobal(self:GetName() .. "Value"):SetText(": " .. tostring(floor((LunarSphereSettings[objectSetting] * 100) + 0.5) / 100));
		end
...
function Lunar.Object.IconPlaceHolder_OnClick(self)
	if (IsControlKeyDown() or GetCursorInfo() ) then
		...
		local typeName = UIDropDownMenu_GetSelectedName(getglobal("LSSettings" .. objectName .. "Type" .. clickType));
...
--				end
		...
		elseif (updateType == "companion") then
...
--]]
		...
		getglobal(self:GetName() .. "Icon"):SetTexture(objectTexture);
		if (buttonType ~= getglobal("LSSettings" .. objectName .. "Type" .. clickType).selectedValue) then
			if (buttonType == 133) then
				UIDropDownMenu_SetText(getglobal("LSSettings" .. objectName .. "Type" .. clickType), Lunar.Locale["BUTTON_INVENTORY4"], getglobal("LSSettings" .. objectName .. "Type" .. clickType));
--				UIDropDownMenu_SetSelectedName(getglobal("LSSettings" .. objectName .. "Type" .. clickType), Lunar.Locale["BUTTON_INVENTORY4"]);
			else
				UIDropDownMenu_SetText(getglobal("LSSettings" .. objectName .. "Type" .. clickType), Lunar.Object.dropdownData["Button_Type"][buttonType + 1][1], getglobal("LSSettings" .. objectName .. "Type" .. clickType));
--				UIDropDownMenu_SetSelectedName(getglobal("LSSettings" .. objectName .. "Type" .. clickType), Lunar.Object.dropdownData["Button_Type"][buttonType + 1][1]);
			end
...
function Lunar.Object.SpeechIconPlaceHolder_OnClick(self)
	...
	if GetCursorInfo() then
		...
		local scriptID = getglobal("LSSettingsscriptSelection").selectedValue or (1)
		if (LunarSpeechLibrary.script[scriptID]) then
			UIDropDownMenu_SetSelectedValue(getglobal("LSSettingsassignmentSelection"), 1);
			getglobal("LSSettingsassignmentSelectionText"):SetText(Lunar.Object.dropdownData["Speech_Assign"][2][1]);
			local objectTexture = string.match(getglobal("LSSettingsassignmentEditButton").actionData or (""), ":::(.*)"); 
...
function Lunar.Object.SphereActionIconPlaceHolder_OnClick(self)
	...
	if GetCursorInfo() then
		...
		if (objectTexture) then
			...
			getglobal("LSSettingssphereActionAssignIcon"):SetTexture("");
		end

		LunarSphereSettings.sphereAction = actionName;

		local obj = getglobal("LSmain");
