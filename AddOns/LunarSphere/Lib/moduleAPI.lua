...
Lunar.API.debugFrameOver = getglobal("LSmain");
...
Lunar.API.debugTooltipUpdater:SetScript("OnUpdate", function(self, arg1)
	...
	if not (Lunar.API.debugTooltipFunction) then
		...
		if (not Lunar.API.debugTooltipFunction) then
			...
			getglobal("LSSettingsshowDebugTooltip"):SetChecked(false);
		end
...
function Lunar.API:ConvertClick(clickName)
	...
	if (clickName == "LeftButton") then
		clickType = 1;
	elseif (clickName == "RightButton") then
		clickType = 2;
...
function Lunar.API:CanFly()
	...
	if (result == true) then
		...
		local hasExpertLicense = GetSpellInfo((GetSpellInfo(34090) or ""))
		local hasFlightMastersLicense = GetSpellInfo((GetSpellInfo(90267) or ""))
		local hasColdFlying = GetSpellInfo((GetSpellInfo(54197) or ""));
		local hasWisdomoftheFourWinds = GetSpellInfo((GetSpellInfo(115913) or ""))
...
--			end
			...
			Lunar.Items.tooltip:SetText(getglobal(name), 1.0, 1.0, 1.0);
			if (subText) then
	...
	for textLine = 1, Lunar.Items.tooltip:NumLines() do
		...
		textContainer = getglobal(Lunar.Items.tooltip:GetName() .. "TextLeft" .. textLine);
		textContainerRight = getglobal(Lunar.Items.tooltip:GetName() .. "TextRight" .. textLine);
...
--[[
	function Lunar.API:RestockReagents(printBill) -- (reagentName, amount, buyAmount)
		...
		-- Set up our locals
		local currentAmount, restockCounter, itemIndex, reagentIndex, totalMerchItems;
		...
		-- Grab how many items the merchant or bank carries carries
		...
		for itemIndex = 1, totalMerchItems do
			...
			-- Grab the current vendor or bank item's info
			if (Lunar.API.isBanker == true) then
				...
				if (itemIndex <= NUM_BANKGENERIC_SLOTS) then
					...
					numAvailable = GetInventoryItemCount("player", currentSlot);
				...
				-- Bank bag item
					...
					_, numAvailable = GetContainerItemInfo(currentBag + NUM_BAG_SLOTS, currentSlot);
			...
			for reagentIndex = 1, table.getn(LunarSphereSettings.reagentList) do
				...
				if (itemName == LunarSphereSettings.reagentList[reagentIndex].name) then
					...
					if (LunarSphereSettings.reagentList[reagentIndex].maxAmount) then
						...
						-- Set the current amount that we're buying to reflect how many units
						-- are in each stack we buy.
						currentAmount = restockCounter * quantity;
...
--				formattedReceipt = string.format("|c00D5B108 %2dg" , formattedReceipt / 10000) .. string.format("|c00C8C5C8 %2ds" , string.sub(formattedReceipt, -4) / 100)..string.format("|c00A3613D %2dc" , string.sub(formattedReceipt, -2));
...
if not (LunarSphereSettings.memoryDisableAHTotals) then
	...
	function Lunar.API:ShowAuctionTotals(toggle)
		...
		if (toggle == true) then
			...
			if (Lunar.API.AuctionTotals:GetParent() == getglobal("AuctionsCancelAuctionButton")) then
	...
	function Lunar.API:CreateAuctionTotals()
		...
		function (self, event)
			...
			if (AuctionFrameAuctions) then
				if (self:GetParent() ~= getglobal("AuctionsCancelAuctionButton")) then
					self:ClearAllPoints()
					self:SetParent("AuctionsCancelAuctionButton");
					self:SetPoint("TOPRIGHT", getglobal("AuctionsCancelAuctionButton"), "TOPLEFT", -2, -4);
	...
	function Lunar.API:ShowBidTotals(toggle)
		...
		if (toggle == true) then
			...
			if (Lunar.API.BidTotals:GetParent() == getglobal("BidBidButton")) then
	...
	function Lunar.API:CreateBidTotals()
		...
		function (self, event)
			...
			if (AuctionFrameBid) then
				if (self:GetParent() ~= getglobal("BidBidButton")) then
					self:ClearAllPoints()
					self:SetParent(getglobal("BidBidButton"));
					self:SetPoint("TOPRIGHT", getglobal("BidBidButton"), "TOPLEFT", -2, -4);
					self:SetFrameStrata("HIGH");
					self:Hide();
					if (self.showMe) then
						self:Show();
...
if not (LunarSphereSettings.memoryDisableDefaultUI) then
	...
	--[[	if not getglobal("LSHideEXP") then
	...
	--		MainMenuXPBarTexture3:SetParent(frame);
		...
		Lunar.API:HideFrame(getglobal("LSHideEXP"), toggle, loading);
	--]]
		if (toggle) then
	...
	--[[	if not getglobal("LSHideMenus") then
		...
		Lunar.API:HideFrame(getglobal("LSHideMenus"), toggle, loading);
--]]    Lunar.API:HideFrame(CompanionsMicroButton, toggle, loading);
		...
		Lunar.API:HideFrame(PVPMicroButton, toggle, loading);
	...
	--[[	if not getglobal("LSHideBags") then
		...
		Lunar.API:HideFrame(getglobal("LSHideBags"), toggle, loading);
	--]]
...
--		Lunar.API:HideFrame(MainMenuBarPerformanceBarFrame, toggle, loading);
	...
	--[[	if not getglobal("LSHideBottomArt") then
		...
		Lunar.API:HideFrame(getglobal("LSHideBottomArt"), toggle, loading);
...
--	end
	...
	function Lunar.API.ShowHideActionButtonGridFunc()
		...
		local index;
		for index = 1, 12 do
--			RegisterStateDriver(getglobal("ActionButton" .. index), "visibility", "hide")
		end
...
--				RegisterStateDriver(getglobal("ActionButton" .. index), "visibility", "show")
				RegisterStateDriver(getglobal("ActionButton" .. index), "visibility", "")
				_G["ActionButton" .. index]:SetAttribute("statehidden", nil);
			elseif (toggle) then
				RegisterStateDriver(getglobal("ActionButton" .. index), "visibility", "hide")
				_G["ActionButton" .. index]:SetAttribute("statehidden", true);
			end
--			Lunar.API:HideFrame(getglobal("ActionButton" .. index), toggle, loading);
		end
...
--				getglobal(buttonName .. "Count"):SetText(DrDamage:CheckAction(getglobal(buttonName), nil, nil, param[0], param[1], param[2]));
				getglobal(buttonName .. "Count"):SetText(DrDamage:CheckAction(nil, nil, nil, spellName, spellRank));
			else
