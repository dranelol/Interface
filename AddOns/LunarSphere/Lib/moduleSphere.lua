...
local dataTracking = {
	...
	eventWatch = {
		[LS_EVENT_MANA] = "UNIT_POWER", --"UNIT_MANA",
		[LS_EVENT_RUNIC] = "UNIT_POWER", --"UNIT_RUNIC_POWER",
		[LS_EVENT_FIVE] = "UNIT_POWER", --"UNIT_MANA",
		[LS_EVENT_HEALTH] = "UNIT_HEALTH",
		[LS_EVENT_EXP] = "PLAYER_XP_UPDATE",
		[LS_EVENT_REP] = "UPDATE_FACTION",
		[LS_EVENT_RAGE] = "UNIT_POWER", --"UNIT_RAGE",
		[LS_EVENT_COMBO] = "UNIT_COMBO_POINTS",
		[LS_EVENT_ENERGY] = "UNIT_POWER", --"UNIT_ENERGY",
		[LS_EVENT_FOCUS] = "UNIT_POWER", --"UNIT_FOCUS",
		[LS_EVENT_EXTRA_POWER] = "UNIT_POWER",
...
--		[LS_EVENT_T_POWER] = "UNIT_POWER", --"UNIT_MANA",
	...
	powerName = {
		[0] = LS_EVENT_MANA,
		[1] = LS_EVENT_RAGE,
		[2] = LS_EVENT_FOCUS, -- focus
		[3] = LS_EVENT_ENERGY,
		[4] = "happiness",
		[5] = LS_EVENT_RUNIC, -- Runes
		[6] = LS_EVENT_RUNIC, -- Runic
	};
...
--	sphereData.overlay:SetToplevel("true");
	...
	sphereData.sphereTextBack:SetTexture(0,0,0,0.7)
...
--		getglobal("LSSettingsXLocationEdit"):SetText(xOfs);
--		getglobal("LSSettingsYLocationEdit"):SetText(yOfs);
...
--			ToggleDropDownMenu(1, nil, getglobal("LSSettingsMenuDropdown"), "LSmain", 0, 0);
			return;
		end
...
--			Lunar.Settings:ToggleSettingsFrame();
		...
		if (Lunar.combatLockdown == false) and (LunarSphereSettings.buttonEditMode == true) then
			if (GetCursorInfo() or (Lunar.Button.updateType) ) then
				...
				Lunar.Button.updateButton = getglobal("LSmain");
...
function Lunar.Sphere:SetSphereTexture(textureType)
	...
	if (LunarSphereSettings.useSphereClickIcon == true) then
		updateSkin = false;
		local clickType = Lunar.Button:GetButtonSetting(0, getglobal("LSmain").currentStance or (Lunar.Button.defaultStance), LUNAR_GET_SHOW_ICON)
		local _, _, _, objectTexture = Lunar.Button:GetButtonData(0, getglobal("LSmain").currentStance or (Lunar.Button.defaultStance), clickType);
		if (objectTexture ~= "") and (objectTexture ~= nil) then
			getglobal("LSsphere"):SetNormalTexture(objectTexture);
		else
			updateSkin = true;	
			getglobal("LSsphere"):SetNormalTexture("Interface\\DialogFrame\\UI-DialogBox-Background");
		end
...
--	if (getglobal("LSSettings" .. gaugeID .. "GaugeColor" .. "Color")) then
--		getglobal("LSSettings" .. gaugeID .. "GaugeColor" .. "Color"):SetTexture(colorR, colorG, colorB);
--	end
...
--	if ( not Lunar.combatLockdown) then
		sphereData.combatGlow:Hide();
		if (LunarSphereSettings.showSphereEditGlow == 1) and (LunarSphereSettings.buttonEditMode == true) then
...
function Lunar.Sphere:SetSphereTextType(eventType, specificID)
	...
	elseif (eventType == LS_EVENT_SPHERE_COOLDOWN) then
		...
		Lunar.Button.UpdateCooldown(getglobal("LSmain"), nil);
...
function Lunar.Sphere:SetOuterGaugeType(eventType, specificID)
	...
	-- "health" done
	-- "five" done
	-- "exp", specificCount (nil for full exp bar, 1 for per-bubble) done
	-- "rep", specificCount (nil for full rep bar, 1 for per-bubble) done

	-- "potions"
	-- "cooldown", specificID

	Lunar.Sphere:SetGaugeType("Outer", eventType, specificID)
--[[	sphereData.gaugeOuterBorder:UnregisterAllEvents();
	...
	-- Although it's listed as the "mana" event, we will watch the proper type
	-- of power type the player uses.
	if (eventType == "mana") then
		eventWatch = dataTracking.powerName[dataTracking.powerType];
	end

	sphereData.gaugeOuterBorder:RegisterEvent(dataTracking.eventWatch[eventWatch]);
	LunarSphereSettings.outerGaugeType = eventType;
	...
	LunarSphereSettings.outerGaugeColor[1] = colorR;
	LunarSphereSettings.outerGaugeColor[2] = colorG;
	LunarSphereSettings.outerGaugeColor[3] = colorB;
	Lunar.Sphere:SetOuterGaugeColor();

	if (getglobal("LSSettings" .. "outerGaugeColor" .. "Color")) then
		getglobal("LSSettings" .. "outerGaugeColor" .. "Color"):SetTexture(colorR, colorG, colorB);
	end

	dataTracking.gaugeOuterStart = nil;
	dataTracking.animateOuter = nil;

	if ((eventType == "mana") or (eventType == "five")) then
		dataTracking.playerMana = UnitMana("player");
	end

	LunarSphereSettings.outerGaugeTypeID = specificID;
...
--[[	sphereData.gaugeInnerBorder:UnregisterAllEvents();

	-- Although it's listed as the "mana" event, we will watch the proper type
	-- of power type the player uses.
	if (eventType == "mana") then
		sphereData.gaugeInnerBorder:RegisterEvent(dataTracking.eventWatch[dataTracking.powerName[dataTracking.powerType] ]);
		LunarSphereSettings.innerGaugeType = "mana"; --eventType; --dataTracking.powerName[dataTracking.powerType];
		Lunar.Sphere:SetInnerGaugeColor(
			gaugeDefaultColor[dataTracking.powerName[dataTracking.powerType] ][1],
			gaugeDefaultColor[dataTracking.powerName[dataTracking.powerType] ][2],
			gaugeDefaultColor[dataTracking.powerName[dataTracking.powerType] ][3])
	else
		sphereData.gaugeInnerBorder:RegisterEvent(dataTracking.eventWatch[eventType]);
		LunarSphereSettings.innerGaugeType = eventType;
		Lunar.Sphere:SetInnerGaugeColor(gaugeDefaultColor[eventType][1], gaugeDefaultColor[eventType][2], gaugeDefaultColor[eventType][3]);
	end
	...
	LunarSphereSettings.innerGaugeTypeID = specificID;
...
--[[	if (LunarSphereSettings.gaugeColor[eventWatch][4] == true) then
	...
	if (getglobal("LSSettings" .. smallID .. "GaugeColor" .. "Color")) then
		getglobal("LSSettings" .. smallID .. "GaugeColor" .. "Color"):SetTexture(colorR, colorG, colorB);
	end
]]--
...
function Lunar.Sphere:UpdateGauge(gaugeType)
	...
	if (checkType == LS_EVENT_POWER) then --or (checkType == "energy") or (checkType == "rage"))   then
		if UnitManaMax("player") > 0 then
			spherePercent =	UnitMana("player") / UnitManaMax("player") * 100;
		else
			spherePercent = 0;
		end
	...
	elseif (checkType == LS_EVENT_T_POWER) then
		if UnitManaMax("target") > 0 then
			spherePercent =	UnitMana("target") / UnitManaMax("target") * 100;
		else
			spherePercent = 0;
		end
	...
	elseif (checkType == LS_EVENT_FIVE) then
		local checkMana = UnitMana("player");
		gaugeStart = nil;
		...
		if (checkTypeID == 1) then
			...
			if UnitPowerMax("player") > 0 then
				if (showAsPercent) then
					sphereText = math.floor(UnitMana("player") / UnitManaMax("player") * 100) .. LunarSphereSettings.sphereTextEnd;
				else
					sphereText = UnitMana("player");
				end
			else
...
function Lunar.Sphere:UpdateGaugeColors()
	...
	for loopCount = 1, 2 do 
		...
		if (getglobal("LSSettings" .. gaugeName .. "GaugeColor" .. "Color")) then
			getglobal("LSSettings" .. gaugeName .. "GaugeColor" .. "Color"):SetTexture(
...
function Lunar.Sphere:ToggleAll(show)
	...
	if (show == 1) then
		...
		getglobal("LunarSphereButtonHeader"):Show();
	else
		sphereData.main:Hide();
		getglobal("LunarSphereButtonHeader"):Hide();
	end
end
...
--		local actionType;
--		_,actionType = Lunar.Button:GetButtonData(0, getglobal("LSmain").currentStance, Lunar.Button:GetButtonSetting(0, getglobal("LSmain").currentStance, LUNAR_GET_SHOW_ICON));
		if (sphereData.main.actionType == "macro") then
			Lunar.Sphere.UpdateMacroEvent();
		end
		if (Lunar.Button.currentMouseOver == 0) then
			Lunar.Button:SetTooltip(getglobal("LSmain"));
		end
		return;
	end
			...
			if (Lunar.Button.currentMouseOver) and ((LunarSphereSettings.buttonEditMode == true) or (LunarSphereSettings.forceDragDrop == true)) then
				if (IsMouseButtonDown("RightButton") )then
					...
					if (Lunar.Button.currentMouseOver > 10) then
						Lunar.Button.updateButton = getglobal("LunarSub" .. Lunar.Button.currentMouseOver .. "Button");
					elseif (Lunar.Button.currentMouseOver == 0) then
						Lunar.Button.updateButton = getglobal("LSmain");
					else						
						Lunar.Button.updateButton = getglobal("LunarMenu" .. Lunar.Button.currentMouseOver .. "Button");
					end
...
--		Lunar.Button:CheckVisibilityOptions()
		...
		for buttonID = 1, 10 do
			menuButton = getglobal("LunarMenu" .. buttonID .. "Button")
...
--					getglobal("LunarMenu" .. buttonID .. "Button"), 
--					getglobal("LunarSub" .. (getglobal("LunarMenu" .. buttonID .. "Button").childID2) .. "Button"),
--					getglobal("LunarMenu" .. buttonID .. "Button").clickType2, 4);
			end
		end
...
--==
	if (event == "PLAYER_ENTERING_WORLD") or (event == "ZONE_CHANGED_NEW_AREA") then --or ("ZONE_CHANGED")
		if (getglobal("LunarMenu1Button")) then
	...
	if ((event == "BAG_CLOSED") and (arg1 > 0) and (arg1 <= 4)) then
		...
		for index = 1, 130 do 
			if (Lunar.Button:GetButtonType(index, stance, 1) == (90 + arg1)) then
				if (index > 10) then
					SetPortraitToTexture(getglobal("LunarSub" .. index .. "ButtonIcon"), getglobal("CharacterBag" .. (arg1 - 1) .. "SlotIconTexture"):GetTexture());
					getglobal("LunarSub" .. index .. "ButtonIcon"):SetVertexColor(1.0, 1.0, 1.0);
					getglobal("LunarSub" .. index .. "ButtonIcon"):SetAlpha(1);
				else
					SetPortraitToTexture(getglobal("LunarMenu" .. index .. "ButtonIcon"), getglobal("CharacterBag" .. (arg1 - 1) .. "SlotIconTexture"):GetTexture());
					getglobal("LunarMenu" .. index .. "ButtonIcon"):SetVertexColor(1.0, 1.0, 1.0);
					getglobal("LunarMenu" .. index .. "ButtonIcon"):SetAlpha(1);
				end
			end
		end
		return;
...
--]]
			...
			getglobal("LSSettings"):ClearAllPoints();
			getglobal("LSSettings"):SetPoint("Center", UIParent, "Center", 0, 200);
