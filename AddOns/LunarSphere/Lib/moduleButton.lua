...
Lunar.Button.swapButton = nil;
Lunar.Button.oldChecked = nil;
Lunar.Button.startSwap = nil;
...
--local menuOffsetY = {28, 6, 0, 0, -6, -28, -6, 0, 0, 6};
...
Lunar.cooldownEffectFunction = {
	[0] = function (button, percent, setup)
		if (setup) then
			...
			getglobal(button:GetName() .. "Icon"):SetAlpha(1);
		end
	end,
...
--		button:SetNormalTexture(LUNAR_ART_PATH .. "buttonSkin_" .. LunarSphereSettings.buttonSkin .. ".blp");
	...
	[3] = function (button, percent, setup)
		...
		getglobal(button:GetName() .. "Icon"):SetAlpha(1 - percent);
	end,
...
--	sphereHeader = CreateFrame("Frame", "LunarSphereButtonHeader", getglobal("LSmain"), "SecureStateHeaderTemplate, SecureStateDriverTemplate")
...
--	sphereMainHeader:SetAttribute("statemap-stance", "$input");
...
--	RegisterStateDriver(sphereMainHeader, "state", "[stance:1] 1; [stance:2] 2; [stance:3] 3; [stance:4] 4; [stance:5] 5; [stance:6] 6; [stance:7] 7; 0");

	sphereMainHeader:SetAttribute("*unit*", "");
--08	sphereMainHeader:SetAttribute("alt-unit*", "player");
	sphereMainHeader:SetAttribute('statebindings', '*:S0;');
--        sphereMainHeader:SetAttribute('childraise', false);
        sphereMainHeader:SetAttribute('childraise', true);

	getglobal("LSmain"):SetParent(sphereMainHeader);

--	sphereHeader = CreateFrame("Frame", "LunarSphereButtonHeader", UIParent, "SecureStateHeaderTemplate, SecureStateDriverTemplate")
--	sphereHeader = CreateFrame("Frame", "LunarSphereButtonHeader", getglobal("LSmain"), "SecureHandlerStateTemplate")
...
--	sphereHeader:SetAttribute("statemap-stance", "$input");
...
--RegisterStateDriver(sphereHeader, "state", "[stance:1] 1; [stance:2] 2; [stance:3] 3; [stance:4] 4; [stance:5] 5; [stance:6] 6; [stance:7] 7; 0");
	

	sphereHeader:SetAttribute("*unit*", "");
--08	sphereHeader:SetAttribute("alt-unit*", "player");
	...
	local sphere = getglobal("LSmain");
	sphere.currentStance = Lunar.Button.defaultStance;

--	sphere:SetAttribute("*statebutton1", "S01");
--	sphere:SetAttribute("*statebutton2", "S02");
--	sphere:SetAttribute("*statebutton3", "S03");
...
--	sphereMainHeader:SetAttribute("addchild", getglobal("LSmain"));
...
--	RegisterStateDriver(newButton, "state", "[stance:1] 1; [stance:2] 2; [stance:3] 3; [stance:4] 4; [stance:5] 5; [stance:6] 6; [stance:7] 7; 0");
...
--				menuHeader:SetAttribute("statemap-stance", "$input");
			else
--	RegisterStateDriver(newButton, "state", "[stance:*] 0; 0");
...
local buttonOnClick = [[
	...
	for index = 1, 10 do 
		...
		if (index == 1) then
			getglobal("LSmain"):SetAttribute("_childupdate", newButton:GetAttribute("_childupdate"));
...
--newButton:SetAttribute("_onstate-state",
...
--	menuHeader:SetAttribute("alt-unit*", "player");
                
		-- Set the points for the headers
--		menuHeader:SetAllPoints(newButton);
	  	menuHeader:RegisterForClicks("LeftButtonUp", "MiddleButtonUp", "RightButtonUp", "Button4Up", "Button5Up");
--		menuHeader:RegisterForClicks("RightButtonUp", "Button4Up", "Button5Up");
                -- events to childs
...
--	RegisterStateDriver(newButton, "state", "[stance:*] 0; 0");
	RegisterStateDriver(menuHeader, "state", "[stance:*] " .. Lunar.Button.defaultStance .."; " .. Lunar.Button.defaultStance);
			...
			-- Otherwise, load it up and show it
				...
				if (LunarSphereSettings.buttonData[index].isMenu) then
					if ((LunarSphereSettings.buttonData[index].menuOpen == true) and not (LunarSphereSettings.buttonData[index].menuUseDelay == 1)) then
						...
						LunarSphereSettings.buttonData[index].menuOpen = nil;
					end
				else
					menuHeader:SetAttribute('state-state', Lunar.Button.defaultStance);  -- + 30);
				end
		...
		newButton:SetFrameLevel(getglobal("LSoverlay"):GetFrameLevel() + 1);
	...
	for menuIndex = 1, 10 do 
		...
		menuButton = getglobal("LunarMenu" .. menuIndex .. "Button")
		...
		for xLoc = 1, 12 do --0, (320 - 32), 32 do 
			...
			getglobal(menuButton:GetName() .. "Header"):WrapScript(newButton, "OnEnter", buttonOnEnter);
			getglobal(menuButton:GetName() .. "Header"):WrapScript(newButton, "OnLeave", buttonOnLeave);
			getglobal(menuButton:GetName() .. "Header"):WrapScript(newButton, "OnClick", buttonOnClick);
...
--					newButton:Show();
			...
			if (LunarSphereSettings.buttonData[menuIndex].isMenu == true) then
				...
				if (LunarSphereSettings.buttonData[index].child == true) then
					LunarSphereSettings.buttonData[index].child = nil;
				end
			end

			newButton:SetParent(getglobal("LunarMenu" .. menuIndex .. "ButtonHeader"));

--			getglobal("LunarMenu" .. menuIndex .. "ButtonHeader"):SetAttribute("addchild", newButton);
--			getglobal("LunarMenu" .. menuIndex .. "Button"):SetAttribute("addchild", newButton);

		end
		Lunar.Button:SetupMenuAngle(menuIndex)
		getglobal("LunarMenu" .. menuIndex .. "ButtonHeader"):SetAttribute("state-state", tostring(getglobal("LunarMenu" .. menuIndex .. "ButtonHeader"):GetAttribute("state") or tostring(Lunar.Button.defaultStance))); 
...
--						if not (LunarSphereSettings.buttonData[menuIndex].useStances) then
--							menuHeader:SetAttribute("state-state", Lunar.Button.defaultStance);
--						else
--							menuHeader:SetAttribute("state-state", GetShapeshiftForm());
--						end
						getglobal(menuButton:GetName() .. "Header"):Execute("toggle = nil; control:RunAttribute('_onclick');");
					else
--						if not (LunarSphereSettings.buttonData[index].useStances) then
--							menuHeader:SetAttribute("state-state", Lunar.Button.defaultStance) -- + 30);
--						else
--							menuHeader:SetAttribute("state-state", GetShapeshiftForm()); -- + 30);
--						end
						getglobal(menuButton:GetName() .. "Header"):Execute("toggle = true; control:RunAttribute('_onclick');");
						LunarSphereSettings.buttonData[menuIndex].menuOpen = nil;
					end
				else
--					menuHeader:SetAttribute('state-state', Lunar.Button.defaultStance);  -- + 30);
				end
	...
	getglobal("LSmain"):SetAttribute('_childupdate', getglobal("LunarMenu1Button"):GetAttribute('_childupdate'));
	sphereMainHeader:SetAttribute('_onclick', getglobal("LunarMenu1ButtonHeader"):GetAttribute('_onclick'));
sphereMainHeader:WrapScript(getglobal("LSmain"), "OnClick",
[[
...
function Lunar.Button:ReloadAllButtons()
	...
	for index = 1, 10 do 
		...
		menuButton = getglobal("LunarMenu" .. index .. "Button");
		menuHeader = getglobal("LunarMenu" .. index .. "ButtonHeader");
...
--				end					
		...
		menuButton:SetFrameLevel(getglobal("LSoverlay"):GetFrameLevel() + 1);
	...
	for menuIndex = 1, 10 do 
		...
		for xLoc = 1, 12 do
			...
			-- Get button
			newButton = getglobal("LunarSub"..index.."Button");
...
--	Lunar.Sphere:SetOuterGaugeColor();
	...
	LunarSphereSettings.buttonEditMode = nil;
	Lunar.Sphere:ShowSphereEditGlow(LunarSphereSettings.showSphereEditGlow);
...
--	if (menuButtonID > mainButtonCount) then
	...
	if (menuButtonID > mainButtonCount) then
		local button = getglobal("LunarMenu" .. menuButtonID .. "Button")
		button:SetAttribute("showstates", "!*");
--		button:Hide();
		getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Hide(); --SetAttribute("state-state", tostring(getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):GetAttribute("state") or tostring(Lunar.Button.defaultStance))); 
--		getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):SetAttribute("state-state", tostring(getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):GetAttribute("state") or tostring(Lunar.Button.defaultStance))); 
		return;
	end
	getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Show(); --SetAttribute("state-state", tostring(getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):GetAttribute("state") or tostring(Lunar.Button.defaultStance))); 
...
--	childScale = (childScale) / buttonScale;

	getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Execute("buttonScale = " .. buttonScale);
	getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Execute("childScale = " .. childScale);
	getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Execute("compressButtons = " .. tostring(LunarSphereSettings.submenuCompression));
	getglobal("LunarMenu" .. menuButtonID .. "ButtonHeader"):Execute("autoShowPet = " .. tostring((LunarSphereSettings.alwaysShowPet == true)));
...
--	local buttonAngle = (menuButtonID - 1) * 36 * ((LunarSphereSettings.buttonSpacing or (100)) / 100)
...
--	local xDir = ((30 * LunarSphereSettings.sphereScale) * math.cos(radians) + (13 * buttonScale) * math.cos(radians) + ) / buttonScale
--	local yDir = ((30 * LunarSphereSettings.sphereScale) * math.sin(radians) + (13 * buttonScale) * math.sin(radians)) / buttonScale
...
--	local yDir = ((30 * LunarSphereSettings.sphereScale) + (13 * buttonScale)) * math.sin(radians) / buttonScale

--	local xDir = (32 + 18 + (LunarSphereSettings.buttonDistance or (0))) / (buttonScale + 0.0) * math.cos(radians) * LunarSphereSettings.sphereScale
--	local yDir = (32 + 18 + (LunarSphereSettings.buttonDistance or (0))) / (buttonScale + 0.0) * math.sin(radians) * LunarSphereSettings.sphereScale

	-- Grab our menu button, set its location around the sphere
	menuButton = getglobal("LunarMenu" .. menuButtonID .. "Button")
	...
	if LunarSphereSettings.buttonData[menuButtonID].detached then
		...
		menuButton:SetPoint("Center", getglobal("LSmain"), "Center", xDir, yDir);
	end
...
--		buttonAngleID = math.floor((buttonAngle + LunarSphereSettings.buttonOffset) / 36) + 1
		if (buttonAngleID < 1) then
--			buttonAngleID = buttonAngleID + 10;
			buttonAngleID = buttonAngleID + mainButtonCount;
		end
--		if (buttonAngleID > 10) then
...
--			buttonAngleID = buttonAngleID - 10;
	...
	for index = 1, 12 do
		...
		-- Grab submenu button
		button = getglobal("LunarSub" .. ((menuButtonID - 1) * 12 + index + 10) .. "Button");
...
--			xMenuDir = xMenuDir + xDir;
--			yMenuDir = yMenuDir + yDir;
		end
		...
		if (index == 1) then
			...
			button:SetPoint("CENTER", getglobal("LunarSub" .. (button:GetID() - 1) .. "Button"), "CENTER", xDir, yDir);
		end
	end		
...
function Lunar.Button:SetupAsMenu(button, clickType, isActive)
	...
	if (button) and (clickType)  then
		if (button:GetID() > 0) and (button:GetID() <= 10)  then
			...
			getglobal(button:GetName() .. "Header"):Execute("menuOpen" .. clickType .. " = " .. tostring(isActive));
--			getglobal(button:GetName() .. "Header"):Execute("menuOpen" .. clickType .. " = " .. tostring(isActive));
--			getglobal(button:GetName() .. "Header"):SetAttribute('state-state', getglobal(button:GetName() .. "Header"):GetAttribute("state"));
		end

--		button:SetAttribute('onmousedownbutton' .. clickType, 'buttondown' .. clickType);
--		button:SetAttribute('onmouseupbutton' .. clickType, 'buttonup' .. clickType);

--		if (isActive == false) then

--			getglobal(button:GetName() .. "Header"):Execute("menuOpen" .. clickType .. " = nil");

			-- Clear the mouse down events
--			button:SetAttribute('anchorchild-buttondown' .. clickType, "");
--			button:SetAttribute('childstate-buttondown' .. clickType, "");
--			button:SetAttribute('childreparent-buttondown' .. clickType, "");
			
			-- Clear the mouse up events
--			button:SetAttribute('anchorchild-buttonup' .. clickType, "");
--			button:SetAttribute('childstate-buttonup' .. clickType, "");
--			button:SetAttribute('childverify-buttonup' .. clickType, "");

--		else

--			getglobal(button:GetName() .. "Header"):Execute("menuOpen" .. clickType .. " = true");

			-- Setup the mouse down events
--			button:SetAttribute('anchorchild-buttondown' .. clickType, getglobal(button:GetName() .. "Header"));
--			button:SetAttribute('childstate-buttondown' .. clickType, "^mousedown");
--			button:SetAttribute('childreparent-buttondown' .. clickType, true);

			-- Setup the mouse up events
--			button:SetAttribute('anchorchild-buttonup' .. clickType, getglobal(button:GetName() .. "Header"));
--			button:SetAttribute('childstate-buttonup' .. clickType, 'mouseup');
--			button:SetAttribute('childverify-buttonup' .. clickType, true);
...
--  * Returns: None
--  *********************
function Lunar.Button:ToggleMenuAutoHide() --toggle)

--	LunarSphereSettings.menuAutoHide = toggle;
	local index, menuHeader;
	for index = 1, 10 do 

		menuHeader = getglobal("LunarMenu" .. index .. "ButtonHeader");

		-- Clear mouse enter/leave, click, and delay support
--		menuHeader:SetAttribute('statemap-anchor-enter', ";");
--		menuHeader:SetAttribute('statemap-anchor-mousedown', ";");
--		menuHeader:SetAttribute("delaystatemap-anchor-leave", ";");
--		menuHeader:SetAttribute('delaystatemap-anchor-mousedown', ";");
--		menuHeader:SetAttribute('delaytimemap-anchor-leave', "");
--		menuHeader:SetAttribute('delaytimemap-anchor', "");
--		menuHeader:SetAttribute("delayhovermap-anchor-leave", "");
--		menuHeader:SetAttribute("delayhovermap-anchor-enter", "");

		menuHeader:Execute("useMenuDelay = nil; menuDelay = nil; useSubmenuClose = nil");
		if (LunarSphereSettings.buttonData[index].menuUseSubmenuClose == 1) then
			menuHeader:Execute("useSubmenuClose = true");
		end

		UnregisterAutoHide(getglobal("LunarSub" .. ((index * 12) - 1) .. "Button"));
...
--[[
local i, button, firstFrame
for i = (index * 12) - 1, (index * 12) + 10 do
	button = getglobal("LunarSub" .. i .. "Button");
	if not firstFrame then
		firstFrame = button;
		RegisterAutoHide(menuHeader, 0.5);
		AddToAutoHide(menuHeader, button);
	else
		AddToAutoHide(menuHeader, button);
	end
end
...
--			menuHeader:SetAttribute('statemap-anchor-mousedown', stateTransitions);
--			menuHeader:SetAttribute("delaystatemap-anchor-leave", stateTransitionsLeave);
--			menuHeader:SetAttribute("delaytimemap-anchor-leave",  "0.5");
--			menuHeader:SetAttribute("delayhovermap-anchor-leave", "true");
--			menuHeader:SetAttribute("delayhovermap-anchor-enter", "true");
--			menuHeader:SetAttribute('state-state', Lunar.Button:GetAssignedStance(getglobal("LunarMenu" ..index .. "Button")) + 30);
...
--			menuHeader:SetAttribute('statemap-anchor-mousedown', stateTransitionsAlwaysShown);
--			menuHeader:SetAttribute('state-state', Lunar.Button:GetAssignedStance(getglobal("LunarMenu" ..index .. "Button")));
		end
...
--			menuHeader:SetAttribute('delaytimemap-anchor-leave', LunarSphereSettings.buttonData[index].menuDelay or ('3'));
--			menuHeader:SetAttribute("delaystatemap-anchor-leave", stateTransitionsLeave);
--			menuHeader:SetAttribute("delayhovermap-anchor-leave", "true");
--			menuHeader:SetAttribute('state-state', Lunar.Button:GetAssignedStance(getglobal("LunarMenu" ..index .. "Button")) + 30);
		end
...
function Lunar.Button:ToggleKeybinds()
	...
	if (hide == true) then
		for index = 1, 10 do
			getglobal("LunarMenu" .. index .. "ButtonHotKey"):Hide()
		end
		for index = 11, 130 do
			getglobal("LunarSub" .. index .. "ButtonHotKey"):Hide()
		end
	else
		for index = 1, 10 do
			getglobal("LunarMenu" .. index .. "ButtonHotKey"):Show()
		end
		for index = 11, 130 do
			getglobal("LunarSub" .. index .. "ButtonHotKey"):Show()
		end
	end
end
...
--	button:SetAttribute("*statebutton1", "S01");
--	button:SetAttribute("*statebutton2", "S02");
--	button:SetAttribute("*statebutton3", "S03");
	button:SetAttribute("checkselfcast", true);
	button:SetAttribute("checkfocuscast", true);
--	button:SetAttribute("useparent-unit", true)
--	button:SetAttribute("*unit*", "");
--08	button:SetAttribute("alt-unit*", "player");
...
-- Add an "*" before the statebuttons above to activate modifiers
	...
	icon = getglobal(name.."Icon");
--	icon:SetDrawLayer("ARTWORK", 0);
	border = getglobal(name.."Border");
	count = getglobal(name.."Count");

	button.texture = icon;

	button.readyShine = getglobal(name .. "Cooldown");
...
--	button.readyShine:SetFrameLevel(1);
--
...
--	drawEdge
--	button.cooldown:SetPoint("TOPLEFT", button, "CENTER", -12.5, 12.5);
	button.cooldown:SetPoint("CENTER", button, "CENTER");
--	button.cooldown:SetWidth(25 * .5);
--	button.cooldown:SetHeight(25 * .5);
...
--	button.procGlow:SetTexture("Interface\\Calendar\\EventNotificationGlow");	-- best
--	button.procGlow:SetTexture("Interface\\Calendar\\UI-Calendar-Button-Glow");
...
--	button.procGlow:SetTexCoord(0.85839844, 0.95214844, 0.30468750, 0.39843750);
--	button.procGlow:SetTexCoord(0.85839844, 0.93652344, 0.06738281, 0.14550781);
--	button.procGlow:SetTexCoord(0.85839844, 0.93652344, 0.06738281, 0.14550781);
	...
	getglobal(name.."HotKey"):SetWidth(48);
	getglobal(name.."HotKey"):ClearAllPoints();
	getglobal(name.."HotKey"):SetPoint("Topright", button, "Topright", 2, -2);
	...
	if (LunarSphereSettings.buttonSkin) then
		...
		button:SetPushedTexture(LUNAR_ART_PATH .. "buttonSkin_" .. LunarSphereSettings.buttonSkin .. ".blp");
		button:SetNormalTexture(LUNAR_ART_PATH .. "buttonSkin_" .. LunarSphereSettings.buttonSkin .. ".blp");
--		if (LunarSphereSettings.buttonSkin == 4) then
...
--	button:SetPushedTexture("Interface\\Minimap\\MiniMap-TrackingBorder.blp");
--	button:SetNormalTexture("Interface\\Minimap\\MiniMap-TrackingBorder.blp");
	button:SetHighlightTexture("Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight");
--	button:SetCheckedTexture("Interface\\Minimap\\UI-Minimap-ZoomButton-Highlight");
...
--	background:SetPoint("CENTER", 13, -14);
...
--[[COOLDOWN EDIT
	-- Create a cooldown object that uses text, set it up, and then hide it for
	-- use at a later time
	button.cooldownTextFrame = CreateFrame("Frame", "", button);
	button.cooldownTextFrame:SetAllPoints(button);
	button.cooldownTextFrame:SetPoint("Center", button, "Center");
--	button.cooldownTextFrame:Show();
	button.cooldownTextFrame:SetAttribute("showstates", "*");

--	if (LunarSphereSettings.debugCooldownFix2 == true) then
--		button.cooldownTextFrame:SetFrameStrata("HIGH");
--	end
	button.cooldownTextFrame:SetFrameLevel(button:GetFrameLevel() + 2);

--	button.cooldownText = tempFrame:CreateFontString(button:GetName() .. "CooldownText", "OVERLAY")
	button.cooldownText = button.cooldownTextFrame:CreateFontString("", "OVERLAY")
...
--	button:SetAttribute("spell", "Abolish Disease")
--	button:SetAttribute("useparent-unit", true)
...
--				self:SetAttribute("shift-type" .. i, "");
--				self:SetAttribute("shift-macrotext" .. i, "");
...
--						self:SetAttribute("shift-macrotext" .. i, "/click PetActionButton" .. (actionName or "1")  .. " RightButton");
--						self:SetAttribute("shift-macrotext" .. i, "/petautocasttoggle " .. (self:GetAttribute("shift-macrotext2-S" .. state .. i) or ("")));
...
--						tempBind = tonumber(bind1);
--						if (tempBind == nil or ((tempBind < 0) or (tempBind > 9)) then
						if (bind1) then
	...
	if (Lunar.combatLockdown == false) and (LunarSphereSettings.buttonEditMode == true) then
		...
		-- Otherwise, we simple start the swapping of buttons
			...
			if (Lunar.Button.startSwap) then
				getglobal(Lunar.Button.swapButton):UnlockHighlight();
				getglobal(Lunar.Button.swapButton):SetChecked(Lunar.Button.oldChecked);
				Lunar.Button.swapButton = nil;
				Lunar.Button.oldChecked = nil;
	...
	if (Lunar.combatLockdown == false) and ((LunarSphereSettings.buttonEditMode == true) or (LunarSphereSettings.forceDragDrop == true)) then
		...
		if (Lunar.Button.swapButton) then
			...
			if (self:GetName() ~= Lunar.Button.swapButton) then
				Lunar.Button:Swap(getglobal(Lunar.Button.swapButton), self);	
			end
			Lunar.Button.startSwap = nil;
			Lunar.Button.swapButton = nil;
			Lunar.Button.oldChecked = nil;
...
--					end
					...
					if (LunarSphereSettings.forceDragDrop) then
						local header;
						if (self:GetID() > 10) then
							header = getglobal("LunarMenu" .. self.parentID .. "ButtonHeader");
						elseif (self:GetID() == 0) then
							header = getglobal("LunarSphereMainButtonHeader");
						else
							header = getglobal("LunarMenu" .. self:GetID() .. "ButtonHeader");
						end
						header:SetAttribute("state-state", header:GetAttribute("state"));
					end

--					if (oldType) then
--						Lunar.Button:SetButtonType(this:GetID(), this.currentStance, 1, 6)
--						this.buttonType = 6;
--						Lunar.Button:SetTooltip(this); -- needed because the button type needs to be readded.
--					end
				end
			end
		end
...
--				Lunar.Settings.buttonEdit = this:GetID();
...
--					local _,_,_,buttonTexture = Lunar.Button:GetButtonData(this:GetID(), stance, clickType)
...
--		local click1, click2, click3;

		-- click 1 check
--		buttonType = Lunar.Button:GetButtonData(self.parentID, stance, 1)
...
--
		...
		for clickType = 1, 3 do
			...
			if (buttonType == 3) or (buttonType == 4) then
				menuButton = getglobal("LunarMenu" .. self.parentID .. "Button");
...
--					menuButton.clickType2 = menuButton.clickType;
--					menuButton.clickID2 = menuButton.clickID;
--					menuButton.clickType = Lunar.API:ConvertClick(arg1);
--					menuButton.childID = self:GetID();
				else
					if (buttonType == 3) and (menuButton.childID) then
						Lunar.Button:UpdateLastUsedSubmenu(menuButton, getglobal("LunarSub" .. menuButton.childID .. "Button"), menuButton.clickType);
					end
					if (buttonType == 4) and (menuButton.childID2) then
						Lunar.Button:UpdateLastUsedSubmenu(menuButton, getglobal("LunarSub" .. menuButton.childID2 .. "Button"), menuButton.clickType2, 4);
					end
				end
				--break;
			end
...
--]]
	...
	-- events
--	if (not IsAltKeyDown()) and (buttonType) then
	...
	if (self:GetID() > 0) and (self:GetID() <= 10) then
		LunarSphereSettings.buttonData[self:GetID()].menuOpen = getglobal(self:GetName() .. "Header"):GetAttribute("toggle");
--		if (tonumber(getglobal(self:GetName() .. "Header"):GetAttribute('state')) < 30) then
--			if (LunarSphereSettings.buttonData[self:GetID()].menuUseDelay == nil) then
--				LunarSphereSettings.buttonData[self:GetID()].menuOpen = true;
--			end
--		end
	end
...
--		end
--	else
--		if (not Lunar.Button.cachedbuttonTypeData) then
--			return nil;
--		end
--	end
...
--				actionType, actionName = string.match(string.match(buttonData, searchString), "(%l)(.*)");
				if (actionType) then
...
--[[
	-- Next, assign the action type and action name

	-- If we don't have an action name, make it blank (a space)
	if (not actionName) or (actionName == "")  then
		actionName = " "
	end

	-- Set the action type to be a formatted type string we need if it exists
	if (actionType) then
		actionType = Lunar.Button.typeData[actionType];
	end
	...
	-- Create the data to be dropped into the database
	if (actionType) then
		buttonData = preString .. actionType .. actionName .. postString;
	else
		buttonData = preString .. actionName .. postString;
	end

	-- If the actionData doesn't exist yet, create it
	if (not LunarSphereSettings.buttonData[buttonID].actionData) then
		LunarSphereSettings.buttonData[buttonID].actionData = "";
	end

	-- If the data for this stance doesn't exist yet, add it
	if not (string.find(LunarSphereSettings.buttonData[buttonID].actionData, searchString)) then
		LunarSphereSettings.buttonData[buttonID].actionData = LunarSphereSettings.buttonData[buttonID].actionData .. Lunar.Button:CreateBlankStanceData();
	end

	-- Now, substitute our new data in place of the old data
	LunarSphereSettings.buttonData[buttonID].actionData = string.gsub(LunarSphereSettings.buttonData[buttonID].actionData, searchString, buttonData);

	-- If the actionTexture doesn't exist yet, create it
	if (not LunarSphereSettings.buttonData[buttonID].actionTexture) then
		LunarSphereSettings.buttonData[buttonID].actionTexture = "";
	end
	
	-- If the data for this stance doesn't exist yet, add it
	if not (string.find(LunarSphereSettings.buttonData[buttonID].actionTexture, searchString)) then
		LunarSphereSettings.buttonData[buttonID].actionTexture = LunarSphereSettings.buttonData[buttonID].actionTexture .. Lunar.Button:CreateBlankStanceData();
	end

	-- Now, substitute our new texture in place of the old texture
	if (buttonTexture == "") then
		buttonTexture = " "
	end
	LunarSphereSettings.buttonData[buttonID].actionTexture = string.gsub(LunarSphereSettings.buttonData[buttonID].actionTexture, searchString, buttonTexture);
--]]
...
function Lunar.Button:UpdateLastUsedSubmenu(menuButton, childButton, clickType, secondToLast)
	...
	--						attributeType = "pet"
						...
						if (updateClick == Lunar.Button:GetButtonSetting(menuButton:GetID(), stance, LUNAR_GET_SHOW_ICON)) then
							if (buttonType == 100) then
								SetPortraitTexture(getglobal(menuButton:GetName() .. "Icon"), "player");
								buttonTexture = "portrait";
							else
								getglobal(menuButton:GetName() .. "Icon"):SetTexture(buttonTexture);
								SetPortraitToTexture(getglobal(menuButton:GetName() .. "Icon"), getglobal(menuButton:GetName() .. "Icon"):GetTexture());
							end
						end
	...
	--					LunarSphereSettings.buttonData[menuButton:GetID()]["texture" .. updateClick] = LunarSphereSettings.buttonData[childButton:GetID()]["texture" .. clickType];
						...
						if (updateClick == Lunar.Button:GetButtonSetting(menuButton:GetID(), stance, LUNAR_GET_SHOW_COUNT)) then
							getglobal(menuButton:GetName() .. "Count"):Hide();
							LunarSphereSettings.buttonData[menuButton:GetID()].showCount = nil;
							...
							elseif (attributeType == "item") then
								...
								if (IsConsumableItem(attributeValue) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString)) then
									getglobal(menuButton:GetName() .. "Count"):SetText(GetItemCount(attributeValue, nil, true));

									if (not LunarSphereSettings.showAssignedCounts == true) then
										getglobal(menuButton:GetName() .. "Count"):Show();
			...
			getglobal(menuButton:GetName() .. "Header"):SetAttribute("state-state", getglobal(menuButton:GetName() .. "Header"):GetAttribute("state"));
	...
	if (buttonType >= 110) and (buttonType < 120) then
		if (Lunar.Button.tradingStage == 3) then
			...
			if UnitExists("target") then
				getglobal("LSbackground"):RegisterEvent("TRADE_CLOSED");
				getglobal("LSbackground"):RegisterEvent("TRADE_REQUEST_CANCEL");
				getglobal("LSbackground"):RegisterEvent("CHAT_MSG_SYSTEM");
				...
				elseif not TradeFrame:IsShown() then
					getglobal("LSbackground"):RegisterEvent("TRADE_SHOW");
...
--		if (this) and (this:GetID() > 0) then
--			if (LunarSphereSettings.buttonData[this:GetID()].isMenu) then
...
--					button:SetAttribute("height", 36);
		...
		getglobal("LunarSphereMainButtonHeader"):Execute("editMode = true");
...
--			UnregisterUnitWatch(button);
			...
			if (index <= mainButtonCount) then
				...
				getglobal(button:GetName() .. "Header"):Execute("editMode = true");
	...
	--			button:SetAttribute("showstates", "*");
				...
				if (not compressionOnly) then
					getglobal("LunarMenu" .. index .. "ButtonHeader"):SetAttribute('state-state', 
					getglobal("LunarMenu" .. index .. "ButtonHeader"):GetAttribute('state'));
				end
...
function Lunar.Button:HideEmptyMenuButtons(compressionOnly)
	...
	getglobal("LunarSphereMainButtonHeader"):Execute("editMode = nil");
	...
	for index = 1, 10 do 
		...
		getglobal(button:GetName() .. "Header"):Execute("editMode = nil");
...
--					button:ClearAllPoints();
					if (index <= 12 * mainButtonCount + 10) then
						if not (math.fmod((index + 1), 12) == 0) then
							button:SetPoint(point, getglobal("LunarSub" .. index - 1 .. "Button"), relativePoint, 0,0)
--							button:SetPoint("Center", getglobal("LunarSub" .. index - 1 .. "Button"), "Center", 0,0)
						else
...
--				end
	...
	--]]
			...
			if (LunarSphereSettings.menuUseDelay == 1) then
				getglobal("LunarMenu" .. index .. "ButtonHeader"):SetAttribute('state-state', Lunar.Button:GetAssignedStance(button) + 30);
			else
				getglobal("LunarMenu" .. index .. "ButtonHeader"):SetAttribute('state-state', 
				getglobal("LunarMenu" .. index .. "ButtonHeader"):GetAttribute('state'));
			end
...
--[[	if (buttonID > 0) and (buttonID <= 10) then
		button = getglobal("LunarMenu" .. buttonID .. "Button");
	...
	elseif (buttonID > 10) then
		button = getglobal("LunarMenu" .. getglobal("LunarSub" .. buttonID .. "Button").parentID .. "Button");
	end
--]]	
...
--			visibleNumber = 1;
--		else
--			settingID = getglobal("LunarSub" .. buttonID .. "Button").parentID;
--			visibleNumber = 2;
--		end
...
--			showState = showState .. "!*";
--		else
--			showState = getglobal("LunarSub" .. (settingID * 12 - 1) .. "Button"):GetAttribute("showstate");
		--end
--	else
	end
	showState = showState .. "!*";
...
function Lunar.Button:CheckVisibilityOptions()
	...
	for index = 1, 10 do
		if (LunarSphereSettings.buttonData[index].buttonTypeData) then
			button = getglobal("LunarMenu" .. index .. "Button");
			offset = 0;
			setState = getglobal(button:GetName() .. "Header"):GetAttribute("state");
...
--			button:Show();
			...
			if (showData == 1) or (showData == 3)  then
				getglobal(button:GetName() .. "Header"):Show();
				button:Show();
			end
			if (showData >= 2) then
				if (LunarSphereSettings.buttonData[index].isMenu) then
					getglobal(button:GetName() .. "Header"):Execute("toggle = nil; control:RunAttribute('_onclick');");
				end
--				setState = button.currentStance;
			...
			if (hideData == 1) or (hideData == 3)  then
				getglobal(button:GetName() .. "Header"):Hide();
				button:Hide();
			end
			if (hideData >= 2) then
				if (LunarSphereSettings.buttonData[index].isMenu) then
					getglobal(button:GetName() .. "Header"):Execute("toggle = true; control:RunAttribute('_onclick');");
				end
--				setState = button.currentStance + 30;
			end
				
--			getglobal(button:GetName() .. "Header"):SetAttribute('state-state', setState);
		end
	end

--	getglobal("LunarSphereButtonHeader"):SetAttribute('state', getglobal("LunarSphereButtonHeader"):GetAttribute('state'));
--	getglobal("LunarSphereMainButtonHeader"):SetAttribute('state', getglobal("LunarSphereMainButtonHeader"):GetAttribute('state'));
...
function Lunar.Button:ShowButtonShine(flag)
	...
	if (flag == 1) then
		...
		for index = 11, buttonCount do
			getglobal("LunarSub" .. index .. "Button").shine:Show();
		end
		for index = 1, 10 do 
			getglobal("LunarMenu" .. index .. "Button").shine:Show();
		end
		...
		-- Cycle through all the sub menu buttons first, then the normal buttons
		for index = 11, buttonCount do
			getglobal("LunarSub" .. index .. "Button").shine:Hide();
		end
		for index = 1, 10 do 
			getglobal("LunarMenu" .. index .. "Button").shine:Hide();
		end
	end
	...
	if (enabled == true) then
		if not (Lunar.Button.enabledButtons == true) then
			...
			for index = 1, buttonCount do
				if (index <= 10) then
					Lunar.Button:SetupStances(getglobal("LunarMenu" .. index .. "Button"));
				else
					Lunar.Button:SetupStances(getglobal("LunarSub" .. index .. "Button"));
				end
			end
		end
	else
		if (Lunar.Button.enabledButtons == true) then
			...
			getglobal("LSmain"):SetAttribute("*statebutton1", "");
			getglobal("LSmain"):SetAttribute("*statebutton2", "");
			getglobal("LSmain"):SetAttribute("*statebutton3", "");
		end
	end
...
function Lunar.Button:ConvertToMenu(self, clickType)
	...
	if (cursorType) and (buttonID <= 10) then
		...
		if (clickType == Lunar.Button:GetButtonSetting(self:GetID(), self.currentStance, LUNAR_GET_SHOW_COUNT)) then
			getglobal(buttonName .. "Count"):Hide();
		end
		getglobal(buttonName .. "Icon"):SetAlpha(1);
...
--		this:SetScript("OnUpdate", nil);
		...
		getglobal(buttonName .. "Icon"):SetVertexColor(1.0, 1.0, 1.0, 1.0);

		-- Reset the rest of the button's data
		LunarSphereSettings.buttonData[buttonID].empty = nil;
		...
		LunarSphereSettings.buttonData[buttonID].child = nil;
...
--		_,_,_,objectTexture = Lunar.Button:GetButtonData(buttonID, GetShapeshiftForm(), 1);
		...
		getglobal(buttonName .. "Icon"):SetTexture(objectTexture);
...
--		this:SetAttribute("spell", ATTRIBUTE_NOOP);

		-- Make our icon fully visible and cut it into a circle
		self:SetAlpha(1);
		SetPortraitToTexture(getglobal(buttonName .. "Icon"), getglobal(buttonName .. "Icon"):GetTexture());
...
--			LunarSphereSettings.buttonData[buttonID]["buttonType" .. clickType] = nil;
		...
		if ((cursorType == "spell") or (cursorType == "item") or (cursorType == "macro") or (cursorType == "companion") or (cursorType == "equipmentset") or (cursorType == "flyout")) then
			buttonType = 1;
			...
			if (buttonID ~= 0) then
				if (clickType == Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_COUNT)) then
					if (objectName == "item:6265") or (IsConsumableItem(objectName) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString)) then
						getglobal(buttonName .. "Count"):SetText(GetItemCount(objectID, nil, true));

						if (not LunarSphereSettings.showAssignedCounts == true) then
							getglobal(buttonName .. "Count"):Show();
...
--		this:SetAttribute("*"..cursorType .. "-S" .. stance .. clickType, Lunar.API:FixFaerie(objectName))
...
function Lunar.Button:WipeMenu(buttonID)
	...
	local index;
	for index = (buttonID * 12) - 1, (buttonID * 12) + 10 do 
		Lunar.Button:Unassign(getglobal("LunarSub" .. index .. "Button"));
		LunarSphereSettings.buttonData[index].child = nil;
	end
end
					...
					if (clickType == Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_COUNT)) then
						if (stackTotal) then
							if (IsConsumableItem(objectName) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString)) then
								getglobal(buttonName .. "Count"):SetText(GetItemCount(objectName, nil, true));
		...
		elseif (buttonType >= 100) and (buttonType < 110) then
			if (buttonType == 100) then
				if (button:GetID() > 0) then
					SetPortraitTexture(getglobal(buttonName .. "Icon"), "player");
					objectTexture = getglobal(buttonName .. "Icon"):GetTexture();
				end
			else
...
--			objectName = Lunar.Button.updateData;
		...
		elseif (buttonType >= 140) and (buttonType < 150)  then
			...
			if ( not isToken ) then
				...
				objectTexture = getglobal(texture);
			end
			cursorType = "pet";
				...
				if (buttonType ~= 100) then
					if not objectTexture then
						if (buttonType < 100) then
							...
							getglobal(buttonName .. "Icon"):SetTexture("Interface\\DialogFrame\\UI-DialogBox-Background");
						end
					else
...
--		if (buttonID > 0) then
		...
		-- If we have an action name, state that our button is no longer empty
		LunarSphereSettings.buttonData[button:GetID()].empty = nil;
...
--				cursorType = "pet"
				...
				button:SetAttribute("*type-S" .. stance  .. clickType, "pet")
				button:SetAttribute("*action-S" .. stance .. clickType, objectName)
--				button:SetAttribute("shift-clickbutton" .. clickType, getglobal("PetActionButton" .. objectName));
...
--		getglobal(button:GetName() .. "Count"):SetText("|cFFFFCC44" .. usedSlots .. "/" .. totalSlots);
		if (usedSlots ~= "?") and (totalSlots ~= "?") and (totalSlots ~= 0) then
			getglobal(button:GetName() .. "Count"):SetText("|cFFFFCC44" .. math.floor(tonumber(usedSlots) / tonumber(totalSlots) * 100) .. "%");
		else
...
function Lunar.Button:Unassign(self, buttonDeassign)
	...
	getglobal(buttonName .. "Icon"):SetTexture("Interface\\DialogFrame\\UI-DialogBox-Background");
	SetPortraitToTexture(getglobal(buttonName .. "Icon"), getglobal(buttonName .. "Icon"):GetTexture());
	getglobal(buttonName .. "Icon"):SetAlpha(0.5);
	button:Hide();
	...
	if (LunarSphereSettings.buttonData[button:GetID()].child == true) then
		getglobal("LunarSub" .. button:GetID() .. "Button"):SetAttribute("showstates", "!*"); --!0,!1");
		if (LunarSphereSettings.submenuCompression) then
			button:SetWidth(4);
			button:SetAttribute("width", 4);
		end
...
--[[ might not need
		if (LunarSphereSettings.buttonData[buttonID].isMenu == true) then
			_,_,_,objectTexture = Lunar.Button:GetButtonData(buttonID, 0, Lunar.Button:GetButtonSetting(buttonID, Lunar.Button:GetAssignedStance(getglobal(buttonName)), LUNAR_GET_SHOW_ICON));
			getglobal(buttonName .. "Icon"):SetTexture(objectTexture);
		end
--]]
...
--						Lunar.Button:FindSpellReagent(getglobal(buttonName), stance, clickType, objectName, cursorType);

						-- Attach its texture to our button
	--					getglobal(buttonName .. "Icon"):SetTexture(LunarSphereSettings.buttonData[buttonID].texture1);
...
--]]
						...
						-- count of the item on the button
						if (stance == Lunar.Button:GetAssignedStance(getglobal(buttonName))) then
							if (clickType == Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_COUNT)) then
								if (buttonID > 0) then
									if (stackTotal) then
										if ((objectName == "item:6265") or IsConsumableItem(objectName) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString) or (LunarSphereSettings.buttonData[buttonID].showCount == true)) then
											getglobal(buttonName .. "Count"):SetText(GetItemCount(objectName));
	--										if (clickType == Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_COUNT)) then
												LunarSphereSettings.buttonData[buttonID].showCount = true;
	--										end
										end
									else
										getglobal(buttonName .. "Count"):SetText(0);
										LunarSphereSettings.buttonData[buttonID].showCount = true;
									end
								end
							end
...
--						LunarSphereSettings.buttonData[buttonID]["texture" .. clickType] = objectTexture;
						getglobal(buttonName).updateIcon = true;
...
--					end
					...
					elseif (buttonType >= 52) and (buttonType < 60)  then
						if (math.fmod(buttonType, 2) == 0) then
							getglobal(buttonName):SetAttribute("target-slot-S" .. stance .. clickType, GetInventorySlotInfo("MainHandSlot"));
						else
							getglobal(buttonName):SetAttribute("target-slot-S" .. stance .. clickType, GetInventorySlotInfo("SecondaryHandSlot"));
						end
					...
					elseif (buttonType >= 120) and (buttonType < 130) then
						...
						elseif (buttonType == 121) then
							getglobal(buttonName):SetAttribute("target-slot-S" .. stance .. clickType, GetInventorySlotInfo("SecondaryHandSlot"));
						elseif (buttonType == 122) then
							getglobal(buttonName):SetAttribute("target-slot-S" .. stance .. clickType, GetInventorySlotInfo("RangedSlot"));
						end
					end

					-- Now set the actual action to be performed
					if (cursorType == "macrotext") then
						getglobal(buttonName):SetAttribute("*type-S" .. stance .. clickType, "macrotext")
						getglobal(buttonName):SetAttribute("*" ..cursorType .. "-S" .. stance .. clickType, objectName)
					else
						if (buttonType >= 140) and (buttonType < 150) then
							getglobal(buttonName):SetAttribute("*type-S" .. stance .. clickType, "pet")
							getglobal(buttonName):SetAttribute("*" .."action" .. "-S" .. stance .. clickType, buttonType - 139)
						elseif (cursorType == "flyout") then
							getglobal(buttonName):SetAttribute("*type-S" .. stance .. clickType, "flyout")
							getglobal(buttonName):SetAttribute("*" .."spell" .. "-S" .. stance .. clickType, objectName)
						else
							getglobal(buttonName):SetAttribute("*type-S" .. stance .. clickType, cursorType)
							getglobal(buttonName):SetAttribute("*" ..cursorType .. "-S" .. stance .. clickType, objectName)
							if (tostring(tonumber(objectName)) == objectName) then
								getglobal(buttonName):SetAttribute("*" ..cursorType .. "2-S" .. stance .. clickType, GetSpellInfo(objectName));
								getglobal(buttonName):SetAttribute("*" ..cursorType .. "-S" .. stance .. clickType, objectName);
							end
						end
					end							
					...
					if (LunarSphereSettings.buttonData[buttonID].child) then
						getglobal(buttonName):SetAttribute("showstates", "0,!30");
					end
				else
					getglobal(buttonName):SetAttribute("*type-S" .. stance .. clickType, "none"); --"spell")
					getglobal(buttonName):SetAttribute("*spell-S" .. stance .. clickType, "")
				end
				...
				if (buttonID > 0) then
					if (stance == Lunar.Button.defaultStance) and (clickType == Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_ICON)) then
						getglobal(buttonName):SetAlpha(1);
						if not (objectTexture) or (not buttonType) then
							...
							if (not buttonType) then
								if not (Lunar.Button:ButtonHasAction(buttonID, stance)) then
									getglobal(buttonName):SetAlpha(0.5);
									getglobal(buttonName .. "Icon"):SetAlpha(0.5);
								end
							end
						end
...
--				buttonType = LunarSphereSettings.buttonData[buttonID]["buttonType" .. clickType];
				if (buttonType) then
					if (buttonType == 2) then -- and (stance == 0) then
						Lunar.Button:SetupAsMenu(getglobal(buttonName), clickType)
					end
					
					-- Self-cast and focus-cast button types will set the target to the player or focus
					if (buttonType == 5) then
						getglobal(buttonName):SetAttribute("unit-S" .. stance .. clickType, "player");
					elseif (buttonType == 6) then
						getglobal(buttonName):SetAttribute("unit-S" .. stance .. clickType, "focus");
					else
						getglobal(buttonName):SetAttribute("unit-S" .. stance .. clickType, "");
					end
					...
					if (buttonType >= 90) and (buttonType <= 95) then
						if (buttonID > 0) then
							if (Lunar.Button:GetButtonSetting(buttonID, stance, LUNAR_GET_SHOW_COUNT) == clickType) then
								Lunar.Button:UpdateBagDisplay(getglobal(buttonName), stance, clickType);
							end
						end
					end
				end
				...
				if (buttonID == 0) then
					getglobal(buttonName):SetAttribute("CTRL-type-S" .. stance .. clickType, "spell")
					getglobal(buttonName):SetAttribute("CTRL-spell-S" .. stance .. clickType, "")
				end

			end

			-- Stance Support turned off
	--		getglobal(buttonName):SetAttribute("statebutton1", "S01");
	--		getglobal(buttonName):SetAttribute("statebutton2", "S02");
	--		getglobal(buttonName):SetAttribute("statebutton3", "S03");

		end

		if (buttonID > 0) then
			-- Set the width of the button
			getglobal(buttonName):SetAttribute("width", 36);
			getglobal(buttonName):SetWidth(36);
		...
		Lunar.Button:SetupStances(getglobal(buttonName));

		-- Update the button's events
		if (buttonID >= 0) then
			getglobal(buttonName).registeredEvents = nil;
			getglobal(buttonName).cooldownID = Lunar.Button:GetButtonSetting(buttonID, Lunar.Button:GetAssignedStance(getglobal(buttonName)), LUNAR_GET_SHOW_COOLDOWN)
			getglobal(buttonName).actionTypeCooldown = nil;
		end

		Lunar.Button:SetupKeybinds(getglobal(buttonName));

		if (buttonID > 0) then
			Lunar.Button:Update(getglobal(buttonName));
		else
			getglobal("LSmain").currentStance = Lunar.Button.defaultStance	
			if not timerData[0] then
				timerData[0] = {};
			end
			timerData[0].cooldownElapsed = 0;
			timerData[0].cooldownTimer = nil;
			getglobal("LSmain"):RegisterEvent("SPELL_UPDATE_COOLDOWN");
			getglobal("LSmain"):RegisterEvent("BAG_UPDATE_COOLDOWN");
			getglobal("LSmain").actionTypeCooldown = nil;
			getglobal("LSmain").actionTypeCount = nil;
			if (LunarSphereSettings.sphereTextType == LS_EVENT_SPHERE_COUNT) then
				Lunar.Sphere:SetSphereText("");
			end
			Lunar.Button:UpdateUsable(getglobal("LSmain"));
			Lunar.Button.UpdateCooldown(getglobal("LSmain"));
			Lunar.Button:UpdateCount(getglobal("LSmain"));
		end
	else
	...
	if (stance == Lunar.Button:GetAssignedStance(button)) and (cursorType == "spell") then
		...
		if (clickType == Lunar.Button:GetButtonSetting(button:GetID(), stance, LUNAR_GET_SHOW_COUNT)) or (clickType == -1) then
			...
			if (spellID) then
				...
				LunarSphereSettings.buttonData[button:GetID()].showCount = nil;
...
function Lunar.Button:GetUseStances(buttonID, useCached)
	...
	if (useCached == true) then
		...
		if (buttonID > 10) then
			useStances = LunarSphereSettings.buttonData[getglobal("LunarSub" .. buttonID .. "Button").parentID].useStances;
		end
	end
...
function Lunar.Button:SetupStances(button)
	...
	if (button:GetID() > 10) then
		header = getglobal("LunarMenu" .. button.parentID .. "ButtonHeader");
	elseif (button:GetID() == 0) then
		header = getglobal("LunarSphereMainButtonHeader");
	else
		header = getglobal(button:GetName() .. "Header");
	end
...
--[[
		if (button:GetID() > 10) then
			getglobal("LunarMenu" .. button.parentID .. "ButtonHeader"):SetAttribute("statemap-stance", "$input");
		end
...
--			button:SetAttribute("*statebutton1", "0:S01;1:S11;2:S21;3:S31;4:S41;5:S51;6:S61;7:S71;8:S81;9:S91;10:S101;11:S111;12:S121;30:S01;31:S11;32:S21;33:S31;34:S41;35:S51;36:S61;37:S71;38:S81;39:S91;40:S101;41:S111;42:S121")
		...
		if (button:GetID() > 10) then
			getglobal("LunarMenu" .. button.parentID .. "ButtonHeader"):SetAttribute("statemap-stance", "");
		end
...
function Lunar.Button:ResetAllButtons()
	...
	for index=1, 10 do 
		...
		getglobal(buttonName):SetAttribute("*spell", "");

		-- Hide the count, remove the checked state...
		getglobal(buttonName .. "Count"):Hide();
		getglobal(buttonName):SetChecked(0);
		...
		Lunar.Button:ResetButtonData(getglobal(buttonName):GetID());
...
Lunar.eventHandler = {
	...
	["PLAYER_AURAS_CHANGED"] = function (self)
...
--		end
	...
	["PET_BAR_UPDATE"] = function (self)
		...
		((self.buttonType == 4) and (self.subButtonType2) and (self.subButtonType2 >= 140) and (self.subButtonType2 < 150)) then
			...
			local icon;
			if (self:GetID() > 0) then
				icon = getglobal(self:GetName() .. "Icon");
				if ( not isToken ) then
				...
				if (LunarSphereSettings.useSphereClickIcon == true) then
					icon = getglobal("LSsphere");
					if ( not isToken ) then
						icon:SetNormalTexture(texture);
					else
						icon:SetNormalTexture(getglobal(texture));
					end
...
--		ActionButton_UpdateHotkeys(this.buttonType1);
	...
	elseif (event == "UPDATE_INVENTORY_ALERTS") then
		...
		timerData[this:GetID()].rangeTimer = -15;
...
--			if (button:GetID() > 0) then
--				getglobal(this:GetName() .. "Icon"):SetTexture(objectTexture);
--				SetPortraitToTexture(getglobal(this:GetName() .. "Icon"), getglobal(this:GetName() .. "Icon"):GetTexture());
--			else
--				if (LunarSphereSettings.useSphereClickIcon == true) then
--					getglobal("LSsphere"):SetNormalTexture(objectTexture);
--					SetPortraitToTexture(getglobal("LSsphere"):GetNormalTexture(), getglobal("LSsphere"):GetNormalTexture():GetTexture());
--				end	
--			end
--			Lunar.Button:SetButtonData(this:GetID(), Lunar.Button.currentStance, clickType, buttonType, nil, nil, objectTexture, true);
		end		
...
--[[
	...
	if ((event == "MODIFIER_STATE_CHANGED") and (this.actionType == "macro")) or (event == "PLAYER_AURAS_CHANGED") or (event == "ACTIONBAR_UPDATE_STATE") or (event == "ACTIONBAR_SLOT_CHANGED") or (event == "UPDATE_SHAPESHIFT_FORM") then --or (event == "PLAYER_TARGET_CHANGED") then
		Lunar.Button:UpdateSpellState(this)
--		local actionType;
----		_,actionType = Lunar.Button:GetButtonData(this:GetID(), GetShapeshiftForm(), Lunar.Button:GetButtonSetting(this:GetID(), GetShapeshiftForm(), LUNAR_GET_SHOW_ICON));

--		_,actionType = Lunar.Button:GetButtonData(this:GetID(), this.currentStance, Lunar.Button:GetButtonSetting(this:GetID(), this.currentStance, LUNAR_GET_SHOW_ICON));
--		actionType = this.actionType
		...
		if (this:GetID() == Lunar.Button.currentMouseOver) then
			Lunar.Button:SetTooltip(this);	
		end
	end
...
function Lunar.Button.UpdateState(self, stateData)
	...
	if (stateData == "state-parent") or ("newstate") then
		...
		if (self:GetID() > 0) then
			self:SetAlpha(1);
			getglobal(self:GetName() .. "Icon"):SetAlpha(1);

			if not (buttonType) then
				if (self:GetID() > 10) then
					self:SetAlpha(0.5);
				end
				getglobal(self:GetName() .. "Icon"):SetAlpha(0.5);
			end

			if (buttonType ~= 100) then
				getglobal(self:GetName() .. "Icon"):SetTexture(buttonTexture);
				SetPortraitToTexture(getglobal(self:GetName() .. "Icon"), getglobal(self:GetName() .. "Icon"):GetTexture());
			else
				SetPortraitTexture(getglobal(self:GetName() .. "Icon"), "player");
			end
		...
		LunarSphereSettings.buttonData[self:GetID()].showCount = nil;
			...
			if (showActionCount) then
				if (showActionCount > 0) then
					...
					if (actionType == "item") then
						...
						if (objectName) then
							if (IsConsumableItem(objectName) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString)) then
				--				getglobal(this:GetName() .. "Count"):SetText(GetItemCount(objectID));
...
--	for clickType = 1, 3 do
--		buttonType = Lunar.Button:GetButtonType(this:GetID(), this.currentStance, clickType);
--		if (buttonType) then
...
--		this.actionName = Lunar.API:FixFaerie(this.actionName);
	...
	--_, actionType, actionName = Lunar.Button:GetButtonData(this:GetID(), this.currentStance, Lunar.Button:GetButtonSetting(this:GetID(), this.currentStance, LUNAR_GET_SHOW_COUNT));
	actionType, actionName = self.actionType, self.actionName;
--	Lunar.Button:FindSpellReagent(this, this.currentStance, Lunar.Button:GetButtonSetting(this:GetID(), this.currentStance, LUNAR_GET_SHOW_COUNT), actionName, actionType)
...
--				this:RegisterEvent("CURRENT_SPELL_CAST_CHANGED");
			...
			if (not self:GetScript("OnShow")) then
				--OLD
				--this:SetScript("OnShow", function() Lunar.Button:UpdateCooldown(this) end);
...
--		this:GetNormalTexture():SetVertexColor(1.0, 1.0, 1.0);
--		this:GetPushedTexture():SetVertexColor(1.0, 1.0, 1.0);
	end				
	...
	if (self.actionType == "item") or (macroEquiped) then
		...
		if ( IsEquippedItem(self.actionName) or (macroEquiped) ) then
			...
			border:Show();
--			this:GetCheckedTexture():SetVertexColor(0,1,0);
--			this:SetChecked(1);
		else
			border:Hide();
--			this:SetChecked(0);
	...
	if (LunarSphereSettings.buttonData[self:GetID()].showCount) then
		getglobal(self:GetName() .. "Count"):Show()
	end
...
function Lunar.Button:UpdatePlayerPortrait()
	...
	if (LunarSphereSettings.useSphereClickIcon ~= true) then
		if (LunarSphereSettings.sphereSkin == Lunar.includedSpheres + 1) then
			Lunar.Sphere:SetSphereTexture();
--			SetPortraitTexture(getglobal("LSsphere"):GetNormalTexture(), "player");
		end
...
--			SetPortraitTexture(getglobal("LSsphere"):GetNormalTexture(), "player");
		...
		if (getglobal("LSmain").buttonType == 100) then
			SetPortraitTexture(getglobal("LSsphere"):GetNormalTexture(), "player");
		end
	end
	...
	for index = 0, 130 do
		...
		if (button) then
			if (button.buttonType == 100) then
				if (index > 10) then
					SetPortraitTexture(getglobal("LunarSub" .. index .. "ButtonIcon"), "player");
				else
					SetPortraitTexture(getglobal("LunarMenu" .. index .. "ButtonIcon"), "player");
				end
			end
		end
	end
...
--	local _, actionType, actionName = Lunar.Button:GetButtonData(this:GetID(), this.currentStance, Lunar.Button:GetButtonSetting(this:GetID(), this.currentStance, LUNAR_GET_SHOW_COUNT));
	...
	if (actionType) then
		...
		if (actionType == "spell") and (self.spellReagent) then
			...
			if (reagentCount > 0) then
				if (self:GetID() > 0) then
					getglobal(self:GetName() .. "Count"):Show();
					getglobal(self:GetName() .. "Count"):SetText(reagentCount);
				else
					if (LunarSphereSettings.sphereTextType == LS_EVENT_SPHERE_COUNT) then
...
--				getglobal(this:GetName() .. "Count"):SetText(itemCount);
...
--				-- If the item is consumable, a reagent, or can stack, we will show the count of the item
--				if (IsConsumableItem(LunarSphereSettings.buttonData[this:GetID()].actionName1) or (stackTotal > 1) or (objectType == Lunar.Items.reagentString) ) then
--					if (totalCharges) then
...
--								end
--							else
--								getglobal(this:GetName() .. "Count"):SetText(totalCharges);
--							end
--						end
--					else
						if (self:GetID() == 0) then
							...
							getglobal(self:GetName() .. "Count"):SetText(itemCount or (0));
							getglobal(self:GetName() .. "Count"):Show();

						end
--					end
--					if (this:GetID() > 0) then
--						getglobal(this:GetName() .. "Count"):Show();
--					end
				else
					if (itemCount == 0) and (objectType == Lunar.Items:GetItemType("consume")) then
						if (self:GetID() == 0) then
							...
							getglobal(self:GetName() .. "Count"):SetText(0);
							getglobal(self:GetName() .. "Count"):Show();
						end
					else
						if (self:GetID() == 0) then
...
--	if (Lunar.updateButtons == 1) and (timerDB) and not (timerDB.cooldownTimer) then
--		return;
--	end
...
--			SetPortraitToTexture(self.texture, self:GetTexture());
		end
	end

	if (self.updatedIcon and not UnitAffectingCombat("player")) then
		self.updatedIcon = nil;
		self:SetAttribute("updateIconName", nil);
		self:SetAttribute("updateIconType", nil);
	end

--[[ DrDamage Support moved to separate function
--	if (this.actionType == "spell") then
--		if Lunar.Button.updateDrDamage then
			getglobal(this:GetName() .. "Count"):Show()
			getglobal(this:GetName() .. "Count"):SetText(DrDamage:CheckAction(nil, nil, nil, nil, this.actionName, 1));
--		elseif (LunarSphereSettings.enableDrDamage ~= true) then
--			getglobal(this:GetName() .. "Count"):Hide()
--		end
	end
--]]
--
...
--[[
		-- Handle the range checker
		if ( (timerData[this:GetID()].rangeTimer) ) then

			-- Update the range timer countdown
			timerData[this:GetID()].rangeTimer = timerData[this:GetID()].rangeTimer - Lunar.Button.updateCounterFrame.elapsed; --elapsed;

			-- If it's time to update our range check..
			if ( timerData[this:GetID()].rangeTimer <= 0 ) then
				Lunar.Button:UpdateUsable(this, nil, true);

				-- Reset the timer
				timerData[this:GetID()].rangeTimer = 0.3 --TOOLTIP_UPDATE_TIME;
			end
		end
--]]
...
--		if (Lunar.Button.updateCounterFrame.updateButtons == 2) then
...
--			if (Lunar.Button.updateCounterFrame.updateButtons) then
...
--						end
						if (LunarSphereSettings.cooldownShowShine == true) then
							CooldownFrame_SetTimer(self.readyShine, 0.1, 0.1, 1)
						end
				...
				-- Otherwise, show the text and update the text contents
					...
					--					button.cooldownTextFrame:SetScale(3/string.len(filteredCooldown));
			--						elseif (string.len(remainingFiltered) > 2) then
			--							button.cooldownText:SetFont(STANDARD_TEXT_FONT, 10.5, "OUTLINE")
					--					button.cooldownTextFrame:SetScale(string.len(filteredCooldown) / 4);
									else
...
--[[
					if (LunarSphereSettings.cooldownEffect > 0) then
--						this.cooldown:Show();
						percent = 1 - (Lunar.Button.updateCounterFrame.currentTime - timerDB.cooldownTimer) / timerDB.cooldownElapsed;
--						this.cooldown:SetWidth(25 * percent);
--						this.cooldown:SetHeight(25 * percent);
						this.cooldown:SetAlpha(percent);
	...
	if (isUsable) then
		...
		if ( inRange == 0 ) then
			if (LunarSphereSettings.vividButtons == true) then
				...
				getglobal(self:GetName() .. "Icon"):SetVertexColor(vividR, vividG, vividB);
			else
				getglobal(self:GetName() .. "Icon"):SetVertexColor(1.0, 0.6, 0.6);
			end
		else
			getglobal(self:GetName() .. "Icon"):SetVertexColor(1.0, 1.0, 1.0);
		end
	...
	elseif ( notEnoughMana ) then
		...
		if ( inRange == 0 ) then
			if (LunarSphereSettings.vividButtons == true) then
				...
				getglobal(self:GetName() .. "Icon"):SetVertexColor(0.8, 0.2, 0.8);
			end
		else
			if (LunarSphereSettings.vividButtons == true) then
				...
				getglobal(self:GetName() .. "Icon"):SetVertexColor(0.5, 0.5, 1.0);
			end
		end
	
	-- Otherwise, we can't use the button at all (maybe we're flying, or shapeshifted
	else
		getglobal(self:GetName() .. "Icon"):SetVertexColor(0.4, 0.4, 0.4);
	end
...
function Lunar.Button:UpdateSkin(skinFile, useColor)
	...
	for index=1, 10 do 
		...
		-- Set the new texture
		getglobal("LunarMenu" .. index .. "Button"):SetPushedTexture(skinFile);
		getglobal("LunarMenu" .. index .. "Button"):SetNormalTexture(skinFile);
--		getglobal("LunarMenu" .. index .. "Button"):SetPushedTexture(LUNAR_ART_PATH .. skinFile);
--		getglobal("LunarMenu" .. index .. "Button"):SetNormalTexture(LUNAR_ART_PATH .. skinFile);

		-- If the use color flag is on, we will paint the button skin to the specified button color
--		if (useColor) then
--			getglobal("LunarMenu" .. index .. "Button"):GetNormalTexture():SetVertexColor(
--				LunarSphereSettings.sphereColor[1], LunarSphereSettings.sphereColor[2], LunarSphereSettings.sphereColor[3], 1);
--			getglobal("LunarMenu" .. index .. "Button"):GetPushedTexture():SetVertexColor(
--				LunarSphereSettings.sphereColor[1], LunarSphereSettings.sphereColor[2], LunarSphereSettings.sphereColor[3], 1);

		-- Otherwise, make it the normal color
--		else
			getglobal("LunarMenu" .. index .. "Button"):GetNormalTexture():SetVertexColor(LunarSphereSettings.buttonColor[1], LunarSphereSettings.buttonColor[2], LunarSphereSettings.buttonColor[3], 1);
			getglobal("LunarMenu" .. index .. "Button"):GetPushedTexture():SetVertexColor(LunarSphereSettings.buttonColor[1], LunarSphereSettings.buttonColor[2], LunarSphereSettings.buttonColor[3], 1);
--		end

		-- If the button happens to be a menu, we paint it the menu button color
		if (LunarSphereSettings.buttonData[index].isMenu) then
			getglobal("LunarMenu" .. index .. "Button"):GetNormalTexture():SetVertexColor(LunarSphereSettings.menuButtonColor[1], LunarSphereSettings.menuButtonColor[2], LunarSphereSettings.menuButtonColor[3]);
			getglobal("LunarMenu" .. index .. "Button"):GetPushedTexture():SetVertexColor(LunarSphereSettings.menuButtonColor[1], LunarSphereSettings.menuButtonColor[2], LunarSphereSettings.menuButtonColor[3]);
		end
	end
	...
	if (buttonCount > 10) then
		for index=11, buttonCount do 
			...
			-- Set the new texture
			getglobal("LunarSub" .. index .. "Button"):SetPushedTexture(skinFile);
			getglobal("LunarSub" .. index .. "Button"):SetNormalTexture(skinFile);
--			getglobal("LunarSub" .. index .. "Button"):SetPushedTexture(LUNAR_ART_PATH .. "" .. skinFile);
--			getglobal("LunarSub" .. index .. "Button"):SetNormalTexture(LUNAR_ART_PATH .. "" .. skinFile);

			-- If the use color flag is on, we will paint the button skin to the specified button color
--			if (useColor) then
--				getglobal("LunarSub" .. index .. "Button"):GetNormalTexture():SetVertexColor(
--					LunarSphereSettings.sphereColor[1], LunarSphereSettings.sphereColor[2], LunarSphereSettings.sphereColor[3], 1);
--				getglobal("LunarSub" .. index .. "Button"):GetPushedTexture():SetVertexColor(
--					LunarSphereSettings.sphereColor[1], LunarSphereSettings.sphereColor[2], LunarSphereSettings.sphereColor[3], 1);

			-- Otherwise, make it the normal color
--			else
				getglobal("LunarSub" .. index .. "Button"):GetNormalTexture():SetVertexColor(LunarSphereSettings.buttonColor[1], LunarSphereSettings.buttonColor[2], LunarSphereSettings.buttonColor[3], 1);
				getglobal("LunarSub" .. index .. "Button"):GetPushedTexture():SetVertexColor(LunarSphereSettings.buttonColor[1], LunarSphereSettings.buttonColor[2], LunarSphereSettings.buttonColor[3], 1);
--			end
...
--]]
		...
		if (button:GetID() > 0) then
			getglobal(button:GetName() .. "Icon"):SetTexture(objectTexture);
...
function Lunar.Button:UpdateButtonColors(menuOnly)
	...
	for index = 1, 10 do 
		if (LunarSphereSettings.buttonData[index].isMenu) then
			getglobal("LunarMenu" .. index .. "Button"):GetNormalTexture():SetVertexColor(unpack(LunarSphereSettings.menuButtonColor));
			getglobal("LunarMenu" .. index .. "Button"):GetPushedTexture():SetVertexColor(unpack(LunarSphereSettings.menuButtonColor));
		else
			getglobal("LunarMenu" .. index .. "Button"):GetNormalTexture():SetVertexColor(unpack(LunarSphereSettings.buttonColor));
			getglobal("LunarMenu" .. index .. "Button"):GetPushedTexture():SetVertexColor(unpack(LunarSphereSettings.buttonColor));
		end
	...
	if (not menuOnly) then
		for index = 11, 130 do 
			getglobal("LunarSub" .. index .. "Button"):GetNormalTexture():SetVertexColor(unpack(LunarSphereSettings.buttonColor));
			getglobal("LunarSub" .. index .. "Button"):GetPushedTexture():SetVertexColor(unpack(LunarSphereSettings.buttonColor));
		end
	end
...
--					Lunar.Debug(swapData.attributeValue);
		...
		getglobal(srcButton:GetName() .. "Icon"):SetTexture("Interface\\DialogFrame\\UI-DialogBox-Background");
		getglobal(destButton:GetName() .. "Icon"):SetTexture("Interface\\DialogFrame\\UI-DialogBox-Background");
		srcButton:SetAlpha(0.5);
		getglobal(srcButton:GetName() .. "Icon"):SetAlpha(0.5);
		destButton:SetAlpha(0.5);
		getglobal(destButton:GetName() .. "Icon"):SetAlpha(0.5);
		...
		if (not LunarSphereSettings.buttonData[srcID].empty) then
			...
			if (buttonTexture) and not (buttonTexture == "") then
				getglobal(srcButton:GetName() .. "Icon"):SetTexture(buttonTexture);
			end
			srcButton:SetAlpha(1);
			getglobal(srcButton:GetName() .. "Icon"):SetAlpha(1);
			srcButton:SetWidth(36);
			srcButton:SetAttribute("width", 36);
...
--			srcButton:SetAttribute("height", 36);
		...
		if (not LunarSphereSettings.buttonData[destID].empty) then
			...
			if (buttonTexture) and not (buttonTexture == "") then
				getglobal(destButton:GetName() .. "Icon"):SetTexture(buttonTexture);
			end
			destButton:SetAlpha(1);
			getglobal(destButton:GetName() .. "Icon"):SetAlpha(1);
			destButton:SetWidth(36);
			destButton:SetAttribute("width", 36);
...
--			destButton:SetAttribute("height", 36);
		...
		SetPortraitToTexture(getglobal(srcButton:GetName() .. "Icon"), getglobal(srcButton:GetName() .. "Icon"):GetTexture());
		SetPortraitToTexture(getglobal(destButton:GetName() .. "Icon"), getglobal(destButton:GetName() .. "Icon"):GetTexture());
		getglobal(srcButton:GetName() .. "Icon"):SetVertexColor(1.0, 1.0, 1.0);
		getglobal(destButton:GetName() .. "Icon"):SetVertexColor(1.0, 1.0, 1.0);
		...
		if (Lunar.Button:GetButtonType(srcID, srcButton.currentStance, Lunar.Button:GetButtonSetting(srcID, srcButton.currentStance, LUNAR_GET_SHOW_ICON)) == 100) then
			SetPortraitTexture(getglobal(srcButton:GetName() .. "Icon"), "player");
		end
		if (Lunar.Button:GetButtonType(destID, destButton.currentStance, Lunar.Button:GetButtonSetting(destID, destButton.currentStance, LUNAR_GET_SHOW_ICON)) == 100) then
			SetPortraitTexture(getglobal(destButton:GetName() .. "Icon"), "player");
		end

		-- Update the button events
		srcButton.registeredEvents = nil;
		destButton.registeredEvents = nil;
		...
		if (srcID > 10) then
			getglobal("LunarMenu" .. srcButton.parentID .. "ButtonHeader"):SetAttribute("_bindingset", "");
			getglobal("LunarMenu" .. srcButton.parentID .. "ButtonHeader"):SetAttribute('state-state', 
			getglobal("LunarMenu" .. srcButton.parentID .. "ButtonHeader"):GetAttribute('state'));
		elseif (srcID == 0) then
			getglobal("LunarSphereMainButtonHeader"):SetAttribute("_bindingset", "");
			getglobal("LunarSphereMainButtonHeader"):SetAttribute('state-state', getglobal("LunarSphereMainButtonHeader"):GetAttribute('state'));
		else		
		...
		if (destID > 10) then
			getglobal("LunarMenu" .. destButton.parentID .. "ButtonHeader"):SetAttribute("_bindingset", "");
			getglobal("LunarMenu" .. destButton.parentID .. "ButtonHeader"):SetAttribute('state-state', 
			getglobal("LunarMenu" .. destButton.parentID .. "ButtonHeader"):GetAttribute('state'));
		elseif (srcID == 0) then
			getglobal("LunarSphereMainButtonHeader"):SetAttribute("_bindingset", "");
			getglobal("LunarSphereMainButtonHeader"):SetAttribute('state-state', getglobal("LunarSphereMainButtonHeader"):GetAttribute('state'));
		else		
		...
		if (swapMenu) then
			...
			for index = 1, 12 do --0 do 
				Lunar.Button:Swap(getglobal("LunarSub" .. tostring(srcID + index) .. "Button"), getglobal("LunarSub" .. tostring(destID + index) .. "Button"), true);
				if (LunarSphereSettings.buttonData[srcID + index].child ~= true) then
					getglobal("LunarSub" .. tostring(srcID + index) .. "Button"):Hide();
--					getglobal("LunarSub" .. tostring(srcID + index) .. "Button"):SetAttribute("showstates", "!0,!1");

				end
				if (LunarSphereSettings.buttonData[destID + index].child ~= true) then
					getglobal("LunarSub" .. tostring(destID + index) .. "Button"):Hide();
--					getglobal("LunarSub" .. tostring(destID + index) .. "Button"):SetAttribute("showstates", "!0,!1");
				end
			end
			
			getglobal("LunarMenu" .. ((srcID + 2) / 12) .. "ButtonHeader"):SetAttribute('state-state', 
			getglobal("LunarMenu" .. ((srcID + 2) / 12) .. "ButtonHeader"):GetAttribute('state'));
			getglobal("LunarMenu" .. ((destID + 2) / 12) .. "ButtonHeader"):SetAttribute('state-state', 
			getglobal("LunarMenu" .. ((destID + 2) / 12) .. "ButtonHeader"):GetAttribute('state'));
	...
	if not Lunar.Button.tooltipSetOwner then
		if (LunarSphereSettings.anchorMode == true) then
			if (Lunar.Button.tooltipCalled ~= true) then
				...
				GameTooltip:SetPoint(Lunar.Button.tooltipPos[9 - pos], getglobal("LSSettingsAnchor"), Lunar.Button.tooltipPos[pos]);
			end
		end
	else
...
--		if ((this:GetID() < 6) or ((this:GetID() > 10) and (this:GetID() < (10 + (12*5))))) then
...
--			GameTooltip_SetDefaultAnchor(GameTooltip, UIParent);
		elseif (tooltipType == 1) then
			GameTooltip:ClearAllPoints();
			GameTooltip:SetPoint(Lunar.Button.tooltipPos[9 - pos], getglobal("LSSettingsLSAnchor"), Lunar.Button.tooltipPos[pos]);
...
--			buttonType = LunarSphereSettings.buttonData[this:GetID()]["buttonType" .. index];
			itemCount = "";
...
-- Simple Tooltip
--				if (index == 2) and (Lunar.Button:GetButtonType(this:GetID(), stance, 3)) then
--					GameTooltip:AddLine(" ", 1, 1, 1);	
--					GameTooltip:AddTexture("Interface\\Addons\\LunarSphere\\art\\tooltipMouse" .. index);
--				end
			end					
...
function Lunar.Button:UpdateBindingText(button)
	...
	getglobal(button:GetName() .. "HotKey"):SetText(keybind);
end
