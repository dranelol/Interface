...
--	Website			: http://www.lunaraddons.com
--
--	Project Start		: April 7, 2007		- Didn't really know what I was getting myself into...
--
--	Project Milestones:	: May 5, 2007		- version 0.100 ready (Beta stage 1.0, "Sphere Test")
...
--		getglobal("LSbackground"):RegisterEvent("CURSOR_UPDATE");
...
--		Lunar.API:CreateMinimapText()
		...
		getglobal("LSmain"):SetPoint(LunarSphereSettings.relativePoint, UIParent, LunarSphereSettings.relativePoint, LunarSphereSettings.xOfs, LunarSphereSettings.yOfs);
...
--		Lunar.Settings.updateScale = GetCVar("useUiScale");
	...
	if (not (versionOK)) or (Lunar.startupMessage) then
		if (LunarSphereSettings.startupMessage) then
			getglobal("LSSettingsStartUpMessage"):SetText(Lunar.startupMessage);
		end
		getglobal("LSSettingsshowStartupMessage"):SetChecked(Lunar.showStartupMessage);
...
--371.5 (new database)
	...
	if (LunarSphereSettings.versionID < 0.523) then
		...
		LunarSphereSettings.debugPortraitFix = nil;
...
function LunarSphere_BackwardsCompatibility_PlayerLogin()
	...
	if (table.getn(playerLoginCheck) > 0) then
		...
		if (playerLoginCheck[1] == 0.712) then
			table.remove(playerLoginCheck, 1);
			local point, relativeTo, relativePoint, xOfs, yOfs = getglobal("LSmain"):GetPoint();
...
function LunarSphere_SlashHandler(msg)
	...
	local obj = getglobal("LSsphere");
	local alpha = 1;
	if (command == "hidesphere") then
		Lunar.Sphere:ToggleSphere(0);
		getglobal("LSSettingssphereAlpha"):SetChecked(true);
	elseif (command == "showsphere") then
		Lunar.Sphere:ToggleSphere(1);
		getglobal("LSSettingssphereAlpha"):SetChecked(false);
...
function LunarSphere_cow()
	...
	if (getglobal("cowFrame")) then
		getglobal("cowFrame"):Hide();
	end

	if (LunarSphereSettings.gievcowlevel) then
		LunarSphereSettings.gievcowlevel = nil;
		if not (getglobal("cowFrame")) then
			...
			for y = 1, 10 do 
				for x = 1, 16 do 
					...
					subFrame:GetNormalTexture():SetTexture(1,1,1);
	...
	--				subFrame:GetNormalTexture():SetVertexColor(x/16, 0, 0);
			...
			cowFrame:SetScript("OnUpdate", function(self, arg1)
				...
				for buttonID = 1, 160 do 
					subFrame = getglobal("cowSubFrame" .. buttonID);
