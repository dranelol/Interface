...
local totalToCancel, totalCancelled, totalCancelable = 0, 0, 0
...
function Manage:ReadyToCancel()
	...
	local function OnUpdate(self, elapsed)
		...
		if( QuickAuctions.db.global.playCancelSound ) then
	...
	frame.text:SetText(L["Ready to cancel an auction, press \"Cancel\"."])
	...
	frame.cancel.tooltip = L["Clicking this will cancel an auction."]
...
function Manage:Cancel(isTest)
	...
	for i=1, GetNumAuctionItems("owner") do
		local name, _, quantity, _, _, _, _, bid, _, buyout, activeBid, highBidder, _, wasSold = GetAuctionItemInfo("owner", i)     
		...
		if( wasSold == 0 and lowestOwner and self:GetBoolConfigValue(itemID, "noCancel") ) then
			if( not tempList[name] and not isTest ) then
				QuickAuctions:Log(name .. "notcancel", string.format(L["Skipped cancelling %s flagged to not be cancelled."], itemLink))
				tempList[name] = true
			end
		...
		elseif( wasSold == 0 and lowestOwner ) then
			...
			if( isPlayer and priceDifference and priceDifference >= priceThreshold ) then
				...
				if( math.floor(lowestBuyout) == math.floor(buyout) ) then
					if( isTest ) then
						cancelableFound = cancelableFound + 1
					else
	...
	-- If we got here, we couldn't find anything to cancel
	if( not isTest ) then
		self:StopCancelling()
	else
		return cancelableFound
	end
end
...
function Manage:QA_STOP_SCAN(event, interrupted)
	...
	if( status.isCancelling ) then
		...
		totalCancelable = self:Cancel(true)
		if( totalCancelable == 0 ) then
			QuickAuctions:Log("cancelstatus", L["Nothing to cancel"])
			self:StopCancelling()
			return
		end
		totalCancelled = totalCancelable
