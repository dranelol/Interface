...
function Scan:AddAuctionRecord(name, link, owner, quantity, bid, buyout)
	...
	buyout = (buyout or 0) / quantity
	bid = (bid or 0) / quantity
...
function Scan:GetPlayerAuctionCount(link, findBuyout, findBid)
	...
	for i=1, GetNumAuctionItems("owner") do
		local name, _, stack, _, _, _, _, bid, _, buyout, _, _, _, wasSold = GetAuctionItemInfo("owner", i)
...
function Scan:ScanAuctions()
	...
	if( status.retries < 3 ) then
		...
		local badData
		for i=1, shown do
			local name, _, _, _, _, _, _, _, _, _, _, _, owner = GetAuctionItemInfo("list", i)     
	...
	for i=1, shown do
		local name, texture, quantity, _, _, _, _, bid, _, buyout, _, _, owner, _, _, hasAllInfo = GetAuctionItemInfo("list", i)     
