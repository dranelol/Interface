...
local browserInsert = {
	...
	func = function()
		...
		MNB.mode = "WM";
		pFrame = MapNotes.DropDown.pFrame;
...
function MNB.ScrollButton_OnEnter(self)
	...
	local key = frame.key or MapNotes.MapKey;
...
function MNB.ScrollButton_OnClick(self, button, down)
	-- printf('MNB.ScrollButton_OnClick(%s)',
	-- 	strjoin(', ', self:GetName(), tostringall(button, down) ));
	...
	if button == "LeftButton" then
		...
		local frameKey = frame.key or MapNotes.MapKey;
		...
		if ( ( MapNotes_Keys[key] ) and ( MapNotes_Keys[key].c ~= nil ) and ( not MNB_Browser.Plugin ) ) then
			...
			frameKey = frame.key or MapNotes.MapKey;	-- update key if map changed
		end
...
function MNB.Marker_OnClick(self, button, down)
	if button == "LeftButton" then
		if ( self.c ) then
...
function MNB.Search_OnClick(self, button, down)
	MNB.mode = "searching";
	MNB.PopulateScroll();
end
...
-- print(tostringall(note.name, note.inf1, note.inf2, note.creator));
-- print(type(note.name), type(note.inf1), type(note.inf2), type(note.creator));

			search = strlower( note.name .. (note.inf1 or '') .. (note.inf2 or '') .. (note.creator or ''));
		...
		if ( nOffset > numNotes ) then
			...
			if ( data.heading ) then
				...
				if ( not noteIcon:GetTexture() ) then
					noteIcon:SetTexture(MAPNOTES_PATH .. 'POIIcons\\Icon' .. data.icon);
				end
