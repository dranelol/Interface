...
local Split = AuctionProfitMaster:NewModule("Split", "AceEvent-3.0")
...
function Split:AuctionHouseClosed()
	if( status.isSplitting ) then
		self:Stop()
		AuctionProfitMaster:Print(L["Stopped splitting due to Auction House frame being closed."])
	end
end

-- This isn't really the most efficient code for doing splits, but as APM requires the user to not be doing anything else
...
function Split:CheckQueueMatch(bag, slot, link, quantity)
	...
	for i=#(splitQueue), 1, -1 do
		...
		if( queue.link == link and queue.quantity == quantity ) then
			...
			AuctionProfitMaster.Post:QueueItem(queue.link, bag, slot)
...
function Split:Stop()
	...
	if( hadSplitFailure ) then
		AuctionProfitMaster:Log(L["Could not post all auctions, ran out of space."])
		AuctionProfitMaster:Print(L["Not all your auctions were posted, ran out of space to split items even after waiting 20 seconds."])
	end

	AuctionProfitMaster.Post:ScanStopped()
end
