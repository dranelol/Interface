...
local Scan = AuctionProfitMaster:NewModule("Scan", "AceEvent-3.0")
...
AuctionProfitMaster.DEBUG_MODE = false
local function debug(...)
	if( AuctionProfitMaster.DEBUG_MODE ) then
...
function Scan:AuctionHouseClosed(postCancel)
	if( status.isScanning ) then
		if not postCancel then
			AuctionProfitMaster:Print(L["Scan interrupted due to Auction House being closed."])
		end
...
function Scan:StartItemScan(filterList)
	...
	self:SendMessage("APM_START_SCAN", "item", #(status.filterList))
	self:SendQuery()
end
...
function Scan:StartCategoryScan(classIndex, subClassList)
	...
	self:SendMessage("APM_START_SCAN", "category")
	self:SendQuery()
end
...
function Scan:StopScanning(interrupted)
	...
	if status.isScanning == "category" and not interrupted then
	...
	self:SendMessage("APM_STOP_SCAN", interrupted)
...
-- Simple data API
APMAPI = {}
function APMAPI:GetData(itemLink)
	if( not itemLink ) then return end
	itemLink = AuctionProfitMaster:GetSafeLink(itemLink)
	
	return auctionData[itemLink]
end

-- Simple craft queue API
function APMAPI:CraftQueue(itemLink, quantity)
	if( not itemLink ) then return end
	itemLink = AuctionProfitMaster:GetSafeLink(itemLink)
	
	AuctionProfitMaster.db.realm.craftQueue[itemLink] = quantity or AuctionProfitMaster.db.realm.craftQueue[itemLink]
	return AuctionProfitMaster.db.realm.craftQueue[itemLink]
end

-- Simple API to empty craft queue
function APMAPI:EmptyQueue()
	for i in pairs(AuctionProfitMaster.db.realm.craftQueue) do
		AuctionProfitMaster.db.realm.craftQueue[i] = nil
	end
end
...
function Scan:AddAuctionRecord(name, link, owner, quantity, bid, buyout)
	...
	link = AuctionProfitMaster:GetNewGem(link) or link

	-- Quick hack job, need to rewrite this all
	if( status.isScanning == "item" ) then
		local ignoreStacks = AuctionProfitMaster.Manage:GetConfigValue(link, "ignoreStacks")
	...
	if( not AuctionProfitMaster.db.factionrealm.player[owner] ) then
	...
	for _, record in pairs(auctionData[link].records) do
		if( record.owner == owner and record.buyout == buyout and record.bid == bid ) then
			...
			record.isPlayer = AuctionProfitMaster.db.factionrealm.player[owner]
			return
		end
	end
	
	-- Nothing available, create a new one
	table.insert(auctionData[link].records, {owner = owner, buyout = buyout, bid = bid, isPlayer = AuctionProfitMaster.db.factionrealm.player[owner], quantity = quantity})
end
...
function Scan:IsLowestAuction(name, buyout, bid)
	...
	for _, record in pairs(auctionData[link].records) do
		...
		if( AuctionProfitMaster.db.factionrealm.whitelist[string.lower(record.owner)] ) then
			if( record.buyout < buyout or ( AuctionProfitMaster.db.global.bidUndercut and record.buyout == buyout and record.bid < bid ) ) then
...
function Scan:GetPlayerAuctionCount(link, findBuyout, findBid)
	...
	for i=1, GetNumAuctionItems("owner") do
		...
		local itemID = AuctionProfitMaster:GetSafeLink(GetAuctionItemLink("owner", i))
...
function Scan:CompareLowestToSecond(link, lowestBuyout)
	...
	if( buyout ) then
		local fallback = AuctionProfitMaster.Manage:GetConfigValue(link, "fallback") * AuctionProfitMaster.Manage:GetConfigValue(link, "fallbackCap")
...
function Scan:GetLowestAuction(link)
	...
	for _, record in pairs(auctionData[link].records) do
		if( not record.isPlayer and record.buyout == buyout ) then
			isPlayer = nil
			if( not AuctionProfitMaster.db.factionrealm.whitelist[string.lower(record.owner)] ) then
				isWhitelist = nil
			end
...
function Scan:ScanAuctions()
	...
	if( status.retries < 3 ) then
		...
		if( badData ) then
			...
			if( status.hardRetry ) then
				...
				self:SendMessage("APM_QUERY_UPDATE", "retry", status.filter, status.page + 1, totalPages, status.retries, 3)
				self:SendQuery()
			-- Soft retry
			else
	...
	for i=1, shown do
		...
		self:AddAuctionRecord(name, AuctionProfitMaster:GetSafeLink(GetAuctionItemLink("list", i)), (owner or ""), quantity, bid, buyout)
		debug("Scanned data", name, owner or "none", quantity, bid, buyout, status.filter)
	end

	-- This query has more pages to scan
	if( shown == NUM_AUCTION_ITEMS_PER_PAGE ) then
		status.page = status.page + 1
		self:SendMessage("APM_QUERY_UPDATE", "page", status.filter, status.page + 1, totalPages)
		self:SendQuery()
		debug("next page", status.page, totalPages, status.filter, status.isScanning, status.classIndex, status.subClassIndex)
		return
	end

	-- Finished with the page
	debug("done with page", status.isScanning)
	self:SendMessage("APM_QUERY_UPDATE", "done", status.filter, totalPages, totalPages)
	...
	if( status.filter ) then
		status.page = 0
		self:SendMessage("APM_QUERY_UPDATE", "next", status.filter, #(status.filterList), status.startFilter)
		self:SendQuery()
		return
	...
	elseif( status.subClassIndex ) then
		status.page = 0
		self:SendMessage("APM_QUERY_UPDATE", "next", status.filter, status.subClassIndex, #(status.subClassIndex), status.startSubClass)
		self:SendQuery()
		return
	end
