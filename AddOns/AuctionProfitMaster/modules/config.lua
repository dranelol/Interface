...
local Config = AuctionProfitMaster:NewModule("Config", "AceEvent-3.0")
...
local function isSoulbound(bag, slot)
	...
	if( not scanTooltip ) then
		scanTooltip = CreateFrame("GameTooltip", "AuctionProfitMasterScanTooltip", UIParent, "GameTooltipTemplate")
	...
	for id=1, scanTooltip:NumLines() do
		local text = _G["AuctionProfitMasterScanTooltipTextLeft" .. id]
...
local function set(info, value)
	AuctionProfitMaster.db.global[info[#(info)]] = value
end

local function get(info, value)
	return AuctionProfitMaster.db.global[info[#(info)]]
end

local function setGroup(info, value)
	local group = info[1] == "general" and "default" or idToGroup[info[2]]
	AuctionProfitMaster.db.profile[info[#(info)]][group] = value
end

local function getGroupSetting(key, group)
	group = idToGroup[group] or group
	if( AuctionProfitMaster.db.profile[key][group] ~= nil ) then
		return AuctionProfitMaster.db.profile[key][group]
	end	
...
local function getGroupMoney(info)
	...
	if( info[1] ~= "general" and AuctionProfitMaster.db.profile[info[#(info)]][idToGroup[info[2]]] == nil ) then
		local money = AuctionProfitMaster:FormatTextMoney(getGroup(info))
		return string.trim(string.gsub(money, "|(.-)([gsc])|r", "%2"))
	end
	
	return string.trim(AuctionProfitMaster:FormatTextMoney(getGroup(info)))
end

local function setGroupOverride(info, value)
	if( not value ) then
		AuctionProfitMaster.db.profile[info.arg][idToGroup[info[2]]] = nil
	else
		AuctionProfitMaster.db.profile[info.arg][idToGroup[info[2]]] = AuctionProfitMaster.db.profile[info.arg].default
	end
end
...
local function getGroupOverride(info)
	...
	return AuctionProfitMaster.db.profile[info.arg][idToGroup[info[2]]] ~= nil
end

local function isGroupOptionDisabled(info)
	return info[1] ~= "general" and AuctionProfitMaster.db.profile[info[#(info)]][idToGroup[info[2]]] == nil
end
...
local function hideHelpText(info)
	return AuctionProfitMaster.db.global.hideHelp
end
...
local groupSettings = {
	...
	quantity = {
		...
		args = {
			...
			overridePerAuctionIsCap= {
				order = 7,
				type = "toggle",
				name = L["Override per auction is a cap"],
				desc = L["Allows you to override the per auction is a cap settings for this group."],
				hidden = hideForDefault,
				disabled = false,
				set = setGroupOverride,
				get = getGroupOverride,
			...
			perAuctionIsCap = {
				order = 8,
				type = "toggle",
				name = L["Use per auction as cap"],
				desc = L["If you don't have enough items for a full post, it will post with what you have."],
			},
		},
	},
	price = {
		...
		args = {
			desc = {
				order = 1,
				type = "description",
				name = function(info) return string.format(L["Undercutting auctions by %s/per item until price goes below %s/per item, unless there is greater than a |cfffed000%.2f%%|r price difference between lowest and second lowest in which case undercutting second lowest auction."], AuctionProfitMaster:FormatTextMoney(getGroupSetting("undercut", info[2])), AuctionProfitMaster:FormatTextMoney(getGroupSetting("threshold", info[2])), getGroupSetting("priceThreshold", info[2]) * 100) end,
				hidden = hideHelpText,
			},
			header = {
				order = 2,
			...
			desc = {
				...
				name = function(info)
					...
					if( autoFallback ) then
						return string.format(L["Once market goes below %s/per item, auctions will be automatically posted at the fallback price of %s/per item."], AuctionProfitMaster:FormatTextMoney(threshold), AuctionProfitMaster:FormatTextMoney(fallback))
					else
						return string.format(L["When no auctions are up, or the market price is above %s/per item auctions will be posted at the fallback price of %s/per item."], AuctionProfitMaster:FormatTextMoney(fallback * fallbackCap), AuctionProfitMaster:FormatTextMoney(fallback))
					end
				end,
				hidden = hideHelpText,
			},
...
local function updateWhitelist()
	...
	local order = 1
	for player, visualName in pairs(AuctionProfitMaster.db.factionrealm.whitelist) do
...
deleteWhitelist = function(info)
	AuctionProfitMaster.db.factionrealm.whitelist[info[#(info)]] = nil
	updateWhitelist()
end
			...
			help = {
				...
				args = {
					help = {
						order = 0,
						type = "description",
						name = function(info) return not AuctionProfitMaster.db.global.superScan and L["Whitelists allow you to set other players besides you and your alts that you do not want to undercut; however, if somebody on your whitelist matches your buyout but lists a lower bid it will still consider them undercutting."] or L["Super scan is enabled, you will not be able to use your whitelist. Disable super scanner to use the whitelist again."] end
					},
				},
			},
			add = {
				...
				args = {
					name = {
						...
						validate = function(info, value)
							...
							for playerID, player in pairs(AuctionProfitMaster.db.factionrealm.whitelist) do
								if( playerID == value ) then
									return string.format(L["The player \"%s\" is already on your whitelist."], player)
								end
							end
							
							for player in pairs(AuctionProfitMaster.db.factionrealm.player) do
						...
						set = function(info, value)
							AuctionProfitMaster.db.factionrealm.whitelist[string.lower(value)] = value
							updateWhitelist()
						end,
						get = false,
					},
...
local addGroupTable = {
	...
	hidden = false,
	func = function(info)
		AuctionProfitMaster.db.factionrealm.mail[idToGroup[info[#(info)]]] = info[2]
		AceRegistry:NotifyChange("AuctionProfitMaster")
	end,
}

local function rebuildMailGroups()
	for id in pairs(addMailItemsTable.args.groups.args) do
		if( not idToGroup[id] or AuctionProfitMaster.db.factionrealm.mail[idToGroup[id]] ) then
			addMailItemsTable.args.groups.args[id] = nil
		end
	end
	
	for id, group in pairs(idToGroup) do
		if( type(group) == "string" and not AuctionProfitMaster.db.factionrealm.mail[group] ) then
...
local addItemTable = {
	...
	width = "half",
	func = function(info)
		AuctionProfitMaster.db.factionrealm.mail[info[#(info)]] = info[2]
...
local function rebuildMailItems()
	for bag=4, 0, -1 do
		for slot=1, GetContainerNumSlots(bag) do
			...
			if( link and not AuctionProfitMaster.db.factionrealm.mail[link] ) then
...
local function massMailItems(info, value)
	...
	for bag=4, 0, -1 do
		for slot=1, GetContainerNumSlots(bag) do
			...
			if( link and name and string.match(name, value) and not AuctionProfitMaster.db.factionrealm.mail[link] and not isSoulbound(bag, slot) ) then
				AuctionProfitMaster.db.factionrealm.mail[link] = info[2]
...
local removeMailGroup = {
	...
	hidden = false,
	func = function(info)
		AuctionProfitMaster.db.factionrealm.mail[idToGroup[info[#(info)]]] = nil
...
local function rebuildMailRemoveGroups(info)
	for id, group in pairs(idToGroup) do
		if( type(group) == "string" and AuctionProfitMaster.db.factionrealm.mail[group] == info[2] ) then
...
local removeMailItem = {
	...
	width = "half",
	func = function(info)
		AuctionProfitMaster.db.factionrealm.mail[info[#(info)]] = nil
		options.args.mail.args[info[2]].args.remove.args.items.args[info[#(info)]] = nil
	end,
}
local function rebuildMailRemoveItems(info)
	for link, target in pairs(AuctionProfitMaster.db.factionrealm.mail) do
		if( target == info[2] and not AuctionProfitMaster.db.global.groups[link] ) then
				...
				help = {
					...
					args = {
						help = {
							order = 1,
							type = "description",
							name = function(info) return string.format(L["Clicking an item or group will stop Auction Profit Master from auto mailing it to %s."], info[2]) end,
						}
					},
				},
				groups = {
...
local function loadMailOptions()
				...
				args = {
					name = {
						...
						validate = function(info, value)
							...
							for _, name in pairs(AuctionProfitMaster.db.factionrealm.mail) do
	...
	for _, target in pairs(AuctionProfitMaster.db.factionrealm.mail) do
...
local removeItemTable = {
	...
	imageWidth = 24,
	func = function(info)
		AuctionProfitMaster.db.global.groups[idToGroup[info[2]]][info[#(info)]] = nil
		options.args.groups.args[info[2]].args.remove.args.list.args[info[#(info)]] = nil
		
		local hasItems
		for itemID in pairs(AuctionProfitMaster.db.global.groups[idToGroup[info[2]]]) do
			hasItems = true
			break
		end
...
local addItem = {
	...
	imageWidth = 24,
	func = function(info)
		AuctionProfitMaster.db.global.groups[idToGroup[info[2]]][info[#(info)]] = true
...
local function deleteGroup(info)
	...
	AuctionProfitMaster.db.global.groups[group] = nil
	for key, data in pairs(AuctionProfitMaster.db.profile) do
...
local function validateGroupRename(info, value)
	value = string.trim(string.lower(value or ""))
	for name in pairs(AuctionProfitMaster.db.global.groups) do
		if( string.lower(name) == value ) then
...
local function renameGroup(info, value)
	...
	local target = AuctionProfitMaster.db.factionrealm.mail[oldName]
	if( target ) then
		AuctionProfitMaster.db.factionrealm.mail[oldName] = nil
		AuctionProfitMaster.db.factionrealm.mail[value] = target
	end
	
	AuctionProfitMaster.db.global.groups[value] = CopyTable(AuctionProfitMaster.db.global.groups[oldName])
	AuctionProfitMaster.db.global.groups[oldName] = nil
	for key, data in pairs(AuctionProfitMaster.db.profile) do
...
local function massAddItems(info, value)
	...
	for bag=4, 0, -1 do
		for slot=1, GetContainerNumSlots(bag) do
			local link = AuctionProfitMaster:GetSafeLink(GetContainerItemLink(bag, slot))
			local name = link and string.lower(GetItemInfo(link))
			if( link and name and string.match(name, value) and not AuctionProfitMaster.modules.Manage.reverseLookup[link] and not isSoulbound(bag, slot) ) then
				AuctionProfitMaster.db.global.groups[idToGroup[info[2]]][link] = true
...
function Config:RebuildItemList(updateOnChange)
	...
	for bag=4, 0, -1 do
		for slot=1, GetContainerNumSlots(bag) do
			local link = AuctionProfitMaster:GetSafeLink(GetContainerItemLink(bag, slot))
			if( link and not addItemsTable.args[link] and not AuctionProfitMaster.modules.Manage.reverseLookup[link] and not isSoulbound(bag, slot) ) then
...
local groupTable = {
	...
	type = "group",
	childGroups = "tab",
	hidden = function(info) return AuctionProfitMaster.db.profile.groupStatus[idToGroup[info[#(info)]]] end,
	name = function(info) return idToGroup[info[#(info)]] end,
	args = {
		general = {
			order = 1,
...
updateGroups = function()
	for id, group in pairs(idToGroup) do
		if( type(group) == "string" and not AuctionProfitMaster.db.global.groups[group] ) then
			...
			add = {
				...
				args = {
					name = {
						...
						set = function(info, value)
							AuctionProfitMaster.db.global.groups[value] = {}
							updateGroups()
						end,
						get = false,
					},
...
local function loadGroupStatus()
	options.args.groupstatus = {
		...
		name = L["Group status"],
		set = function(info, value)
			AuctionProfitMaster.db.profile.groupStatus[idToGroup[info[#(info)]]] = not value and true or nil
		end,
		get = function(info)
			return not AuctionProfitMaster.db.profile.groupStatus[idToGroup[info[#(info)]]]
		end,
		args = {
			help = {
				order = 0,
				...
				args = {
					help = {
						type = "description",
						name = L["Enabling groups will cause them to be active and usable in this profile. If you disable them you will no longer see them in the configuration menu, and they will not be managed by Auction Profit Master.|n|nFor all intents and purposes, they do not exist in Auction Profit Master."],
					}
				}
			},
			list = {
...
local function loadOptions()
	...
	options.args.profile = LibStub("AceDBOptions-3.0"):GetOptionsTable(AuctionProfitMaster.db, true)
	options.args.profile.order = 1.5
end

SLASH_APM1 = nil

SLASH_AuctionProfitMaster1 = "/apm" 
SLASH_AuctionProfitMaster2 = "/AuctionProfitMaster"
SlashCmdList["AuctionProfitMaster"] = function(msg)
	...
	if( cmd == "cancel" ) then
		if( not arg or arg == "" ) then
			AuctionProfitMaster:Print(L["You did not seem to enter any filters to cancel with. If you really meant to cancel everything, use /apm cancelall"])
		elseif( AuctionFrame and AuctionFrame:IsVisible() ) then
			AuctionProfitMaster.Manage:CancelMatch(arg)
		else
			AuctionProfitMaster:Print(L["Cannot cancel auctions without the Auction House window open."])
		end
	-- Mass cancel
		...
		if( AuctionFrame and AuctionFrame:IsVisible() ) then
			...
			if( tonumber(parsedArg) ) then
				...
				if( parsedArg ~= 12 and parsedArg ~= 2 ) then
					AuctionProfitMaster:Print(string.format(L["Invalid time entered, should either be 12 or 2 you entered \"%s\""], parsedArg))
					return
				end
			...
			elseif( parsedArg ~= "" ) then
				...
				if( not groupName ) then
					AuctionProfitMaster:Print(string.format(L["No group named %s exists."], arg))
					return
				end
			end
			
			AuctionProfitMaster.Manage:CancelAll(groupName, cancelTime, cancelBelow)
		else
			AuctionProfitMaster:Print(L["Cannot cancel auctions without the Auction House window open."])
		end
	-- Configuration
	elseif( cmd == "config" ) then
		if( not AceDialog and not AceRegistry ) then
			...
			LibStub("AceConfig-3.0"):RegisterOptionsTable("AuctionProfitMaster", options)
			AceDialog:SetDefaultSize("AuctionProfitMaster", 700, 500)
		end
			
		AceDialog:Open("AuctionProfitMaster")
	-- Tradeskill
	elseif( cmd == "tradeskill" ) then
		if( AuctionProfitMaster.Tradeskill.frame and AuctionProfitMaster.Tradeskill.frame:IsVisible() ) then
			AuctionProfitMaster.Tradeskill.frame:Hide()
		else
			AuctionProfitMaster.Tradeskill:CreateFrame()
			AuctionProfitMaster.Tradeskill.frame:Show()
		end
	...
	elseif( cmd == "summary" ) then
		...
		AuctionProfitMaster:Print(L["Slash commands"])
		AuctionProfitMaster:Echo(L["/apm cancelall <group/12/2> - Cancels all active auctions, or cancels auctions in a group if you pass one, or cancels auctions with less than 12 or 2 hours left."])
		AuctionProfitMaster:Echo(L["/apm cancel <match> - Cancels all auctions that match the entered filter, if you enter glyph it will cancel everything with \"glyph\" in it."])
		AuctionProfitMaster:Echo(L["/apm summary - Shows the auction summary"])
		AuctionProfitMaster:Echo(L["/apm tradeskill - Toggles showing the craft queue window for tradeskills"])
		AuctionProfitMaster:Echo(L["/apm config - Toggles the configuration"])
	end
end
