...
local Mail = AuctionProfitMaster:NewModule("Mail", "AceEvent-3.0")
local L = AuctionProfitMaster.L

local eventThrottle = CreateFrame("Frame", nil, MailFrame)
local reverseLookup = AuctionProfitMaster.modules.Manage.reverseLookup
...
function Mail:OnInitialize()
	...
	-- Don't show mass opening if Postal is enabled since postals button will block APMs
	...
	local check = CreateFrame("Button", "AuctionProfitMasterSendMail", SendMailFrame, "UIPanelButtonTemplate")
	check:SetHeight(26)
	check:SetWidth(250)
	check:SetText("Auction Profit Master: " .. L["Auto mail"])
	...
	check.tooltip = L["Enables Auction Profit Master auto mailer, the last patch of mails will take ~10 seconds to send.\n\n[WARNING!] You will not get any confirmation before it starts to send mails, it is your own fault if you mistype your bankers name."]
	
	if AuctionProfitMaster.db.char.autoMail then
		AuctionProfitMaster.db.char.autoMail = nil
		-- Popup Confirmation Window used in this module
		StaticPopupDialogs["APM_AUTOMAIL"] = {
			text = "Because of changes made in 4.0.1, APM's Auto-Mailing is now started by a button on the 'Send Mail' tab of the mail frame.",
			button1 = "OK",
			timeout = 0,
			whileDead = true,
			hideOnEscape = false,
		}
		StaticPopup_Show("APM_AUTOMAIL")
	end
	...
	cacheFrame:SetScript("OnUpdate", function(self, elapsed)
		if( not waitingForData ) then
			...
			if( seconds <= 0 ) then
				...
				if( AuctionProfitMaster.db.global.autoCheck ) then
					waitingForData = true
...
function Mail:StartAutoLooting()
	...
	if( AuctionProfitMaster.db.global.autoCheck and autoLootTotal == 0 and total > 0 ) then
...
function Mail:StopAutoLooting(failed)
	if( failed ) then
		AuctionProfitMaster:Print(L["Cannot finish auto looting, inventory is full or too many unique items."])
	end
...
function Mail:MAIL_INBOX_UPDATE()
	...
	if( (not self.massOpening:IsEnabled()) and autoLootTotal ~= current ) then
		...
		if( AuctionProfitMaster.db.global.autoCheck and current == 0 and total > 0 ) then
			self.massOpening:SetText(L["Waiting..."])
		elseif( current == 0 and ( not AuctionProfitMaster.db.global.autoCheck or total == 0 ) ) then
...
function Mail:TargetHasItems(checkLocks)
	for bag=0, 4 do
		for slot=1, GetContainerNumSlots(bag) do
			...
			local target = AuctionProfitMaster.db.factionrealm.mail[link] or reverseLookup[link] and AuctionProfitMaster.db.factionrealm.mail[reverseLookup[link]]
...
function Mail:Start()
	...
	if( activeMailTarget ) then
		...
		AuctionProfitMaster:Print(L["No items to send."])
	end
...
function Mail:SendMail()
	...
	AuctionProfitMaster:Print(string.format(L["Auto mailed items off to %s!"], activeMailTarget))
