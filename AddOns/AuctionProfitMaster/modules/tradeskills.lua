...
local Tradeskill = AuctionProfitMaster:NewModule("Tradeskill", "AceEvent-3.0")
...
function Tradeskill:Update()
	...
	for id, itemid in pairs(itemList) do
		if( tradeList[itemid] and AuctionProfitMaster.db.realm.craftQueue[itemid] ) then
	...
	if( #(rowDisplay) > 0 ) then
		...
		for itemid, needed in pairs(materials) do
			local itemCount = AuctionProfitMaster.Manage:GetNumInBags(itemid)
	...
	for index, data in pairs(rowDisplay) do
		if( index >= offset and displayIndex < MAX_ROWS ) then
			...
			if( type(data) == "number" ) then	
				...
				row:SetFormattedText("%s [%d]", itemName, AuctionProfitMaster.db.realm.craftQueue[itemid])
				row.itemID = itemid
			else
				row:SetText(data)
				row.itemID = nil
...
function Tradeskill:BuyMaterials()
	for i=1, GetMerchantNumItems() do
		local link = AuctionProfitMaster:GetSafeLink(GetMerchantItemLink(i))
		if( materials[link] ) then
			local maxStack = GetMerchantItemMaxStack(i)
			local toBuy = materials[link] - AuctionProfitMaster.Manage:GetNumInBags(link)
			while( toBuy > 0 ) do
...
function Tradeskill:RebuildList()
	...
	for itemid in pairs(AuctionProfitMaster.db.realm.craftQueue) do
...
function Tradeskill:CreateFrame()
	...
	self.button:SetText("APM")
	self.button.tooltip = L["Click to view Auction Profit Master tradeskill queue"]
	self.button:SetPoint("TOPRIGHT", TradeSkillFrame, "TOPRIGHT", -70, -2)
	...
	self.buy:SetPoint("TOPLEFT", TradeSkillFrame, "TOPLEFT", 60, -2)
	...
	-- Scroll frame
	self.frame.scroll = CreateFrame("ScrollFrame", "APMCraftGUIScroll", self.frame, "FauxScrollFrameTemplate")
	...
	local function craftItem(self)
		...
		for i=1, GetNumTradeSkills() do
			local itemid = AuctionProfitMaster:GetSafeLink(GetTradeSkillItemLink(i)) or Tradeskill:GetEnchantItemID(i)
			if( itemid == self.itemID and AuctionProfitMaster.db.realm.craftQueue[itemid] ) then
				-- Make sure we don't wait for it to create more than we can
				local createCap = select(3, GetTradeSkillInfo(i))
				local quantity = math.min(AuctionProfitMaster.db.realm.craftQueue[itemid], createCap)
...
do
	...
	function Tradeskill:TradeskillUpdate()
		...
		AuctionProfitMaster.db.realm.crafts[professions[GetTradeSkillLine()]] = true
		...
		-- Record list
		for i=1, GetNumTradeSkills() do
			local itemid = AuctionProfitMaster:GetSafeLink(GetTradeSkillItemLink(i))
			if( itemid ) then
				local enchantid = string.match(GetTradeSkillRecipeLink(i), "enchant:([0-9]+)")
				AuctionProfitMaster.db.realm.crafts[itemid] = tonumber(enchantid) or true
				
				-- Create a list of items we need to create this item
				if( AuctionProfitMaster.db.realm.craftQueue[itemid] ) then
					for rID=1, GetTradeSkillNumReagents(i) do
						local perOne = select(3, GetTradeSkillReagentInfo(i, rID))
						local link = AuctionProfitMaster:GetSafeLink(GetTradeSkillReagentItemLink(i, rID))
						if( link ) then
							materials[link] = (materials[link] or 0) + (AuctionProfitMaster.db.realm.craftQueue[itemid] * perOne)
						end
					end
				end
...
function Tradeskill:UNIT_SPELLCAST_SUCCEEDED(event, unit, name)
	if name == "Glyph of Arcane Intellect" then name = "Glyph of Arcane Brilliance" end
	if( unit == "player" and name == creatingItem and AuctionProfitMaster.db.realm.craftQueue[creatingItemID] ) then
		AuctionProfitMaster.db.realm.craftQueue[creatingItemID] = AuctionProfitMaster.db.realm.craftQueue[creatingItemID] - 1

		if( AuctionProfitMaster.db.realm.craftQueue[creatingItemID] <= 0 ) then
			AuctionProfitMaster.db.realm.craftQueue[creatingItemID] = nil
