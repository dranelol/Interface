...
local Summary = AuctionProfitMaster:NewModule("Summary", "AceEvent-3.0")
...
function Summary:GetData(type)
	if( not AuctionFrame or not AuctionFrame:IsVisible() ) then
		AuctionProfitMaster:Print(L["Auction House must be visible for you to use this."])
		return
	end
	...
	if( not classIndex ) then
		AuctionProfitMaster:Print(string.format(L["Cannot find class index. APM still needs to be localized into %s for this feature to work."], GetLocale()))
		return
	end
	...
	AuctionProfitMaster.Scan:StartCategoryScan(classIndex, subClassList)
end

function Summary:APM_START_SCAN()
	self.getDataButton:Disable()
	self.stopButton:Enable()
end

function Summary:APM_QUERY_UPDATE(event, type, filter, ...)
...
-- We got all the data!
function Summary:APM_STOP_SCAN(event, interrupted)
...
function Summary:UpdateItemData(summaryData, name, quantity, link, itemLevel, itemType, subType, stackCount)
	...
	if( summaryData.groupedBy ~= "parent" and type(AuctionProfitMaster.db.realm.crafts[link]) == "number" ) then
		enchantLink = string.format("enchant:%d", AuctionProfitMaster.db.realm.crafts[link])
	end
		
	local row = displayData[index]
	local lowestBuyout, lowestBid, lowestOwner, isWhitelist, isPlayer = AuctionProfitMaster.Scan:GetLowestAuction(link)
	row.enabled = true
	row.name = name
...
function Summary:CompileData()
	...
	if( not AuctionProfitMaster.db.global.summaryItems[selectedSummary] ) then
		AuctionProfitMaster.db.global.summaryItems[selectedSummary] = {}
	end
	...
	for link, data in pairs(AuctionProfitMaster.Scan.auctionData) do
		...
		if( name and data.quantity > 0 and ( not summaryData.itemType or summaryData.itemType == itemType ) and ( not summaryData.notSubType or summaryData.notSubType ~= subType ) and ( not summaryData.subType or summaryData.subType == subType ) ) then
			if not (AuctionProfitMaster:IsOldGem(link) or usedLinks[link]) then
				usedLinks[link] = true
				AuctionProfitMaster.db.global.summaryItems[selectedSummary][link] = true
	...
	for link in pairs(AuctionProfitMaster.db.global.summaryItems[selectedSummary]) do
		local name, _, _, itemLevel, _, itemType, subType, stackCount = GetItemInfo(link)
		if not (AuctionProfitMaster:IsOldGem(link) or usedLinks[link]) then
			usedLinks[link] = true
			...
			if( name and ( not summaryData.itemType or summaryData.itemType == itemType ) and ( not summaryData.notSubType or summaryData.notSubType ~= subType ) and ( not summaryData.subType or summaryData.subType == subType ) ) then
				...
				AuctionProfitMaster.db.global.summaryItems[selectedSummary][link] = nil
			end
		end
	end
...
function Summary:Update()
	...
	for index, data in pairs(displayData) do
		...
		if( data.enabled and data.isParent ) then
			...
			if( not AuctionProfitMaster.db.profile.categories[data.name] ) then
				for index, childData in pairs(displayData) do
					if( childData.enabled and not childData.isParent and childData.parent == data.name ) then
						if( not summaryData.canCraft or not AuctionProfitMaster.db.global.hideUncraft or summaryData.canCraft(childData.link, (GetItemInfo(childData.link)) or "") ) then
							table.insert(rowDisplay, index)
						end
					end
				end
	...
	for index, dataID in pairs(rowDisplay) do
		if( index >= offset and displayIndex < MAX_SUMMARY_ROWS ) then
			...
			if( data.quantity ) then
				local inventory = AuctionProfitMaster.Manage:GetNumInBags(data.link) > 0 and string.format("(%d) ", AuctionProfitMaster.Manage:GetNumInBags(data.link)) or ""
				local activeNumber = AuctionProfitMaster.Scan:GetPlayerItemQuantity(data.link)
				local active = ""
				
				row.tooltipData = string.format(L["\n\n%d in inventory\n%d on the Auction House"], AuctionProfitMaster.Manage:GetNumInBags(data.link), data.quantity)
				
				if( activeNumber > 0 ) then
					local postCap = AuctionProfitMaster.Manage:GetConfigValue(data.link, "postCap")
			...
			if( data.buyout and data.buyout > 0 ) then
				row.buyout:SetText(data.buyout and AuctionProfitMaster:FormatTextMoney(data.buyout, true) or "")
			...
			-- Orrr a child
				...
				local craftQuantity = ""
				local craftData = AuctionProfitMaster.db.realm.craftQueue[data.link] or AuctionProfitMaster.db.realm.craftQueue[data.enchantLink]
				if( craftData ) then
...
function Summary:CreateGUI()
	...
	self.frame = CreateFrame("Frame", "AuctionProfitMasterSummaryGUI", UIParent)
	...
	table.insert(UISpecialFrames, "AuctionProfitMasterSummaryGUI")
	...
	title:SetText(L["Auction Profit Master"])
	...
	-- Date scroll frame
	self.middleFrame.scroll = CreateFrame("ScrollFrame", "APMSummaryGUIScrollMiddle", self.frame, "FauxScrollFrameTemplate")
	...
	row:SetText(L["Stop"])
	row:SetScript("OnClick", function()
		AuctionProfitMaster.Scan:StopScanning(true)
	end)
	...
	row:SetHighlightFontObject(GameFontHighlightSmall)
	row:SetDisabledFontObject(GameFontDisableSmall)
	row:SetText(AuctionProfitMaster.db.global.hideUncraft and L["Show uncraftables"] or L["Hide uncraftables"])
	row:SetScript("OnEnter", showTooltip)
	row:SetScript("OnLeave", hideTooltip)
	row:SetScript("OnClick", function(self)
		AuctionProfitMaster.db.global.hideUncraft = not AuctionProfitMaster.db.global.hideUncraft
		self:SetText(AuctionProfitMaster.db.global.hideUncraft and L["Show uncraftables"] or L["Hide uncraftables"])
		Summary:Update()
	end)
	...
	row:SetScript("OnLeave", hideTooltip)
	row:SetScript("OnClick", function()
		for k in pairs(AuctionProfitMaster.db.realm.craftQueue) do
			AuctionProfitMaster.db.realm.craftQueue[k] = nil
		end
	...
	local function toggleCategory(self)
		if( self.parent ) then
			AuctionProfitMaster.db.profile.categories[self.parent] = not AuctionProfitMaster.db.profile.categories[self.parent]
			Summary:Update()
		end
	end
	...
	local function rowClicked(self, mouseButton)
		...
		elseif( not self.disableCraftQueue and mouseButton == "LeftButton" and self.baseLink and not IsModifierKeyDown() ) then
			AuctionProfitMaster.db.realm.craftQueue[self.baseLink] = (AuctionProfitMaster.db.realm.craftQueue[self.baseLink] or 0) + 1
			Summary:Update()
		elseif( not self.disableCraftQueue and mouseButton == "RightButton" and self.baseLink and not IsModifierKeyDown() ) then
			if( AuctionProfitMaster.db.realm.craftQueue[self.baseLink] and AuctionProfitMaster.db.realm.craftQueue[self.baseLink] > 1 ) then
				AuctionProfitMaster.db.realm.craftQueue[self.baseLink] = AuctionProfitMaster.db.realm.craftQueue[self.baseLink] - 1
			else
				AuctionProfitMaster.db.realm.craftQueue[self.baseLink] = nil
			end
			Summary:Update()
		end
	end
	...
	local function OnKeyDown(self, key)
		...
		-- Number pad uses NUMPAD# instead of just # so strip out the NUMPAD portion
		key = string.gsub(key, "NUMPAD", "")
	
		-- Enter pressed, unfocus
		if( key == "ENTER" ) then
			AuctionProfitMaster.db.realm.craftQueue[self.baseLink] = tonumber(createQuantity)
			if( AuctionProfitMaster.db.realm.craftQueue[self.baseLink] <= 0 ) then
				AuctionProfitMaster.db.realm.craftQueue[self.baseLink] = nil
			end
				
			if( AuctionProfitMaster.Tradeskill.frame and AuctionProfitMaster.Tradeskill.frame:IsVisible() ) then
				AuctionProfitMaster.Tradeskill:RebuildList()
				AuctionProfitMaster.Tradeskill:TradeskillUpdate()
			end
	...
	local function OnDoubleClick(self, mouse)
		for i=1, 4 do
			rowClicked(self, mouse)
		end
		return
		...
		-- createQuantity = AuctionProfitMaster.db.realm.craftQueue[self.baseLink]
	...
	summaryCats = {
		["Gems"] = {
			...
			canCraft = function(link, name) 
				if( not AuctionProfitMaster.db.realm.crafts.Jewelcrafter ) then return true end
				
				if( AuctionProfitMaster.db.realm.crafts[link] ) then
					return true
		...
		["Flasks"] = {
			text = L["Flasks"],
			subType = L["Flask"],
			canCraft = function(link, name) if( not AuctionProfitMaster.db.realm.crafts.Alchemy ) then return true else return AuctionProfitMaster.db.realm.crafts[link] end end,
			groupedBy = "itemLevel",
			auctionClass = L["Consumable"],
		...
		["Potions"] = {
			...
			groupedBy = "itemLevel",
			auctionClass = L["Consumable"],
			auctionSubClass = {[L["Potion"]] = true},
		},
		["Food"] = {
			text = L["Food"],
			canCraft = function(link, name) if( not AuctionProfitMaster.db.realm.crafts.Cook ) then return true else return AuctionProfitMaster.db.realm.crafts[link] end end,
		...
		["Glyphs"] = {
			text = L["Glyphs"],
			itemType = L["Glyph"],
			canCraft = function(link, name) if( not AuctionProfitMaster.db.realm.crafts.Scribe ) then return true else return AuctionProfitMaster.db.realm.crafts[link] end end,
			groupedBy = "subType",
