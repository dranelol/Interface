...
local childIterator = function(header, i)
	i = i + 1;
    ...
    if not frame or (frame == i) then
      ...
      if f.secure then
        ...
        if var.general.debuffs then
          f:SetAttribute("filter", "HARMFUL")
        else
          f:SetAttribute("filter", "HELPFUL")
        end
        f:SetAttribute("template", "SecureActionButtonTemplate")

        f:SetAttribute("point", "TOPRIGHT")
        f:SetAttribute("xOffset", 0)
        f:SetAttribute("yOffset", -18)

        f:SetAttribute("sortMethod", "INDEX")
...
sbf.GetBuffFrame = function(self, var)
  local f
  if var.general.interactiveFrame and (var.general.unit == "player") then
    for i,frame in pairs(secureFrames) do
    ...
    f = CreateFrame("Frame", "SBFFrame"..frameCount, UIParent, "SBFBuffFrameTemplate")
    f.secure = true
    f.ActiveChildren = SecureFrameActiveChildren
  else
...
sbf.FillFrame = function(self, frame)
  ...
  for i=1,frame._var.layout.count do
    slot = self:PopulateSlot(frame._var)
    slot.index = i
    if frame._var.general.interactiveFrame then
      if slot.icon then
        slot.icon:SetAttribute("index", i)
        slot.icon:SetAttribute("unit", frame._var.general.unit)
      end
      if slot.bar then
        slot.bar:SetAttribute("index", i)
        slot.bar:SetAttribute("unit", frame._var.general.unit)
      end
      if slot.name then
        slot.name:SetAttribute("index", i)
        slot.name:SetAttribute("unit", frame._var.general.unit)
      end
    end
...
sbf.PopulateSlot = function(self, var)
  ...
  if var.icon then
    slot.icon = self:GetIcon(var.general.interactiveFrame)
    slot.icon.start = 0
  ...
  if var.bar then
    slot.bar = self:GetBar(var.general.interactiveFrame)
    slot.bar:Show()
  end
  if var.name then
    slot.name = self:GetName(var.general.interactiveFrame)
  end
  if var.count then
...
sbf.ShowBuffs = function(self, unit, f)
  ...
  for i,frame in pairs(self.frames) do
    if not f or (i == frame) then
      if (frame.unit == unit) or (not frame.unit and unit == "player") then
...
local SetNormalVertexColor = function(Button, r, g, b, a)
   ...
   elseif (skinnerName == "Masque") then
      local Region = skinnerLib:GetNormal(Button)
      if Region then Region:SetVertexColor(r, g, b, a) end
   end
end
...
sbf.FrameShowBuffs = function(self, frame)
  ...
  for j,slot in ipairs(frame.slots) do
    ...
    if buff then
      ...
      if slot.icon then
        slot.icon:HideGraphics()
      end
      if slot.name then
        slot.name:HideGraphics()
      end
      if slot.bar then
        slot.bar:HideGraphics()
      end
    end
    if self.showingOptions then
