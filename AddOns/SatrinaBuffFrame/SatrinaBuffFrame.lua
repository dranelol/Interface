-- $Date: 2012-09-12 04:52:12 +0000 (Wed, 12 Sep 2012) $
...
sbf.revision = tonumber(("$Revision: 145 $"):match("%d+"))
...
local delayedCallList = {}
sbf.DelayedCall = function(self, func, count)
  count = count or 1
  for k,v in pairs(delayedCallList) do
    if (v[1] == func) then
      if (v[2] < count) then
        v[2] = count
      end
  ...
  tinsert(delayedCallList, {func, count})
end
...
SBF.OnUpdate = function(self, e)
  ...
  if not sbf.showingOptions then
    ...
    if (#delayedCallList > 0) then
      tinsert(delayedCallList, {nil, nil})
      local func = tremove(delayedCallList, 1)
      while func and (func[1] ~= nil) do
        func[2] = func[2] - 1
        if (func[2] < 1) then
          sbf[func[1]](sbf)
        else
          tinsert(delayedCallList, func)
        end
        func = tremove(delayedCallList, 1)
      end
    end
