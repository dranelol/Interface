...
function Gladius:SendAnnouncement(text, color)
	...
	local dest = db.announceType
	
	if( dest == "rw" and not IsRaidLeader() and not IsRaidOfficer() and GetNumRaidMembers() > 0 ) then
		dest = "party"
	end
	
	-- party chat
	if ( dest == "party" and (GetNumPartyMembers() > 0 or GetNumRaidMembers() > 0) ) then
