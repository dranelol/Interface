...
sbf.ValidateFrameVars = function(self, i, create)
  ...
  v.layout = v.layout or {}
    ...
    v.layout.sort = v.layout.sort or "INDEX"
	elseif (i == 2) then
  ...
  -- Interactive frame limitations
  if v.general.interactiveFrame then
    v.layout.visibility = 1
    v.layout.sort = "INDEX"
    v.layout.direction = "+"
    ...
    if v.icon then
      v.icon.disableRightClick = nil
    end
    if v.bar then
      v.bar.disableRightClick = nil
    end
    if (#v.filters > 0) then
      v.filters = {}
    end
...
sbf.DoGeneralSavedVars = function(self, v, new, frameNum)
  ...
  if new then
    if frameNum == 1 then
      ...
      v.general.interactiveFrame= true
...
sbf.minorUpdate = function(self)
  if self.db.global.version and self.db.global.version < 3.2 then
  ...
  if not self.db.global.minor or self.db.global.minor ~= 0 then
    self.db.global.message = true
    self.db.global.minor = self.minor
  end
end
...
sbf.message = {
  "This version of SBF is beta testing the new way of clicking off buffs",
  "When you set a frame as interactive, several configuration options will be unavailable for that frame",
  "Most notably, filtering, visibility, flowing, and sorting",
  "Known issue: names do not show in interactive frames",
